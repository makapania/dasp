...
class TestDecodeSolution:
    ...
    def test_decodes_preprocessing(self):
        ...
        n_wavelengths = 100
        chromosome = np.zeros(8 + n_wavelengths, dtype=int)  # 8 config genes now
    ...
    def test_decodes_model_type(self):
        ...
        n_wavelengths = 100
        chromosome = np.zeros(8 + n_wavelengths, dtype=int)  # 8 config genes now
        chromosome[8:] = 1  # Select all wavelengths (start at gene 8)
    ...
    def test_decodes_wavelength_selection(self):
        ...
        for idx in selected_indices:
            chromosome[8 + idx] = 1  # Wavelengths start at gene 8
    ...
    def test_decodes_model_params(self):
        ...
        n_wavelengths = 100
        chromosome = np.zeros(8 + n_wavelengths, dtype=int)  # 8 config genes now
        chromosome[8:] = 1  # All wavelengths (start at gene 8)
...
class TestR2Computation:
    ...
    def test_computes_r2_for_solution(self, synthetic_spectra_small):
        ...
        n_wavelengths = X_array.shape[1]
        solution = np.zeros(8 + n_wavelengths, dtype=int)  # 8 config genes now
        ...
        solution[8:] = 1  # all wavelengths (start at gene 8)
