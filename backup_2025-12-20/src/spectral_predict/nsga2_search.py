...
# Optional imports for tree-based models (V1 imports these unconditionally but we handle gracefully)
try:
...
def _build_model(model_type: str, model_param: int, task_type: str, random_state: int, hyperparams: Optional[Dict[str, float]] = None):
    ...
    if model_type == 'PLS':
        ...
        if task_type == 'regression':
            ...
            return SVC(kernel=kernel, C=C, random_state=random_state, probability=True)
        ...
        if not HAS_NEURAL_BOOSTED or NeuralBoostedRegressor is None:
            return None
...
def decode_solution(chromosome: np.ndarray, n_wavelengths: int, model_types: Optional[List[str]] = None, task_type: str = 'regression') -> Dict[str, Any]:
    ...
    if model_type == 'PLS':
        ...
        nsga_overrides = {'n_estimators': n_estimators, 'max_depth': max_depth, 'n_jobs': 1}
    elif model_type == 'LightGBM':
        ...
        nsga_overrides = {'kernel': kernel, 'C': C}
    elif model_type == 'MLP':
...
def convert_nsga2_to_v1_format(
    ...
    for i, (objectives, solution) in enumerate(zip(
        ...
        row = {
            ...
            'SubsetTag': 'nsga2',
            'Imbalance': 'none',
            'top_vars': _indices_to_wavelength_str(decoded['selected_indices'][:30], wavelengths) if decoded['selected_indices'] else 'N/A',
