...
{sample_weight_block_cv}    # _fit_fold mirrors in-app cv_utils._fit_with_early_stopping: boosters
    ...
    # return. Mirrors the regression template's pattern at
    # templates/validation.py:103 (and PR #26's same-branch addition for the
    # plain classification template). The imbalance-aware classification CV
...
# ============================================================================
# T-31 Multi-Class SIMCA / class-modeling reproduction export (Phase D3)
#
# The class-modeling paradigm (per-class membership engines -> a decision
# matrix, not a single estimator + CV score) does not fit the generic
# section pipeline above. Instead we emit a small self-contained script that
# calls the same backend (``build_multiclass_decision_view``) with the exact
# config, so the exported decision matrix is bit-identical to the in-app one by
# construction.
# ============================================================================

_MULTICLASS_REPRODUCTION_HEADER = '''#!/usr/bin/env python3
"""Reproduction script for a dasp Multi-Class SIMCA (class-modeling) decision matrix.

Auto-generated by Spectral Predict. Reproduces the in-app per-sample decision
matrix bit-identically via the spectral_predict backend. Requires the
spectral_predict package to be importable.
"""
import base64
import json
...
def _multiclass_data_section(data_X=None, data_y=None, wavelengths=None) -> str:
    """Return the ``X``/``y`` setup section for the reproduction script.

    Embeds the data (float64 base64 + JSON labels) when provided, otherwise a
    TODO placeholder for the user to fill in.
    """
    if data_X is not None and data_y is not None:
        ...
        if wavelengths is not None:
            ...
            wl_list = [str(c) for c in data_X.columns]
        else:
            wl_list = list(range(X_arr.shape[1]))
        x_b64 = base64.b64encode(np.ascontiguousarray(X_arr).tobytes()).decode('ascii')
        return (
            "# --- Data (embedded) ---\n"
    ...
    return (
        "# --- Data ---\n"
        "# TODO: load your spectra into X (n_samples x n_wavelengths) and the\n"
        "# class labels into y. Column headers of X should be the wavelengths.\n"
        "# X = pd.read_csv('spectra.csv', index_col=0)\n"
        "# y = pd.read_csv('labels.csv', index_col=0).iloc[:, 0]\n"
        "raise NotImplementedError('Fill in the X / y loading section above.')\n"
    )
...
view = build_multiclass_decision_view(
    ...
    min_class_samples=CONFIG["min_class_samples"],
    variable_selection=CONFIG["variable_selection"],
    n_select=CONFIG["n_select"],
)

if view["reason"]:
    raise RuntimeError("Decision view failed: " + view["reason"])
...
for i, sid in enumerate(view["sample_ids"]):
    ...
    row = {{"Sample": sid,
            "TrueClass": view["true_labels"][i],
            "Decision": view["labels"][i]}}
    for j, c in enumerate(classes):
        row["p(%s)" % c] = float(P[i, j])
    row["Accepted"] = ", ".join(str(a) for a in accepted)
    rows.append(row)

decision_matrix = pd.DataFrame(rows)
print(decision_matrix.to_string(index=False))
decision_matrix.to_csv("decision_matrix.csv", index=False)
print("\\nDecision counts:",
      {{k: int((decision_matrix["Decision"] == k).sum())
        for k in list(classes) + ["multiple", "novel"]}})
'''
...
def generate_multiclass_reproduction_script(config: Dict[str, Any],
                                            data_X=None,
                                            data_y=None,
                                            wavelengths=None) -> str:
    """Generate a standalone script reproducing a multi-class decision matrix.

    Parameters
    ----------
    config : dict
        The ``view["config"]`` dict from
        :func:`spectral_predict.search.build_multiclass_decision_view`
        (engine, alpha, n_components, scaling, min_class_samples,
        variable_selection, n_select, preprocess_cfg).
    ...
    return _MULTICLASS_REPRODUCTION_HEADER + body


def generate_multiclass_reproduction_notebook(config: Dict[str, Any],
                                              data_X=None,
                                              data_y=None,
    ...
    cells = [
        {
            ...
            "metadata": {},
            "source": [
                "# Multi-Class SIMCA — Decision Matrix Reproduction\n",
                "\n",
                "Auto-generated by Spectral Predict. Reproduces the in-app "
                "per-sample decision matrix via the `spectral_predict` backend.",
            ],
        },
        {
            "cell_type": "code",
            ...
            "metadata": {},
            "outputs": [],
            "source": (setup + body).splitlines(keepends=True),
        },
    ]
    return {
        "cells": cells,
