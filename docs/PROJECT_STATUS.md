...
> ## ▶ ACTIVE DIRECTION (2026-07-02) — T-17 "FULL SEARCH PARITY" — design APPROVED, handoff written, implementation NOT started.
> User tested the shipped Multi-Target tab and found it runs each model ONCE, default hyperparameters, full spectra, one preprocessing — **no search**. Decision: give the multi-target path full parity with the single-Y search (grid over **preprocessing × varsel × model × hyperparameters**, joint multi-Y CV, ranked by joint Q², real leaderboard). Brainstorming done + design approved. **Self-contained agent handoff: `docs/CONTINUATION_PROMPT_2026-07-02_T17-full-parity.md`** — start there. Key decisions: new orchestrator reusing existing pieces (`run_search` UNTOUCHED); inherit the user's existing search config (overrides only if needed, start with none); UVE/CARS skip-with-notice; grow on PR #63 branch (do NOT merge foundation first); runtime = worker-thread + joblib + progress + Cancel (no cap). Next agent: write spec → user review → `writing-plans` → implement → re-review + re-gate. GUI bug fixes already landed (`52c814b`): multi-target results horizontal scroll + a pre-existing Refine-tab CV-toggle crash.
>
