...
> **NEXT: C2** `run_multiclass_simca_search` (patterned on `run_one_class_search` :5594 — preprocessing × engine grid, per-class n_components auto-tuned inside each row via Phase-A `cross_validate`/`evaluate_novelty`, rows keyed (preprocessing, engine, varsel_path), NaN-safe α-sweep-AUC ranking, unmodelable class → row flagged not dropped) → **C3** result-row population (engine_family/varsel_path per row) → Phase-C multi-family gate + real-data e2e (user's `Contaminated Samples Raw_ORAU Added.xlsx`, LOCO on held-out `Site`) → Phase D (GUI + export). Do NOT merge until Phases C–D + merge-gate. Full detail: SESSION_LOG 2026-07-04.
>
> ---
>
