...
> **Last updated:** 2026-05-08 (later same day) — **PR #56 review-cycle 1 (Codex) closed.** Four findings addressed on top of the existing T-CI-1 commits; all targeted tests green locally. STRONG #2 produced a real CLI refactor: `cli.py` now defers `pandas`, `interactive_gui`, `interactive`, `search`, and `report` imports until after `argparse.parse_args`, so `--version` and `--help` never trigger `matplotlib.use('TkAgg')`. The `tests/test_run_logging.py:366` headless-Linux skip was removed; the test now runs cross-platform. Verified `from spectral_predict import cli` followed by `cli.main()` with `argv=['--version']` succeeds even when `matplotlib` and `tkinter` are blocked from `sys.meta_path`. STRONG #1 was the hard one: the deadlocking GUI test could not be identified by static inspection (no `mainloop()` / `wait_window` / `wait_variable` calls in `tests/gui/`; tests use `run_analysis_direct()` which bypasses Tk threading) and Xvfb is unavailable on Windows for live reproduction. Restored Linux GUI coverage as an **informational `gui-linux` job** with `continue-on-error: true`, `pytest-timeout==2.1.0` (added to `[dev]` extras), and `--timeout=180 --timeout-method=thread`. Next CI run will surface the hanging test by name in the gui-linux logs without blocking merges. **Filed T-CI-2** for closure: identify hanger from gui-linux output, quarantine with `@pytest.mark.skip(reason="T-CI-2")`, drop `--ignore=tests/gui` from the main test job, flip `continue-on-error` to `false`. MEDIUM #3: `tests/test_io_spc.py` — dispatcher routing now proven via monkeypatched sentinel exception (preferred Codex path); the loose `"spc"` substring fallback was also dropped from the parser-error fragment list. MEDIUM #4: `test_agilent_not_implemented` deleted (the production reader is real, the not-implemented contract was stale) and replaced with `test_agilent_invalid_file_raises_value_error` asserting parser-level `ValueError` on a deliberately invalid `.seq` file (skip if optional dep absent).
>
> **Test results (worktree, .venv312, PYTHONPATH-overridden to worktree src):** `tests/test_run_logging.py` 23/23 PASS; `tests/test_io_spc.py` 11 pass + 2 conditional skips; `tests/test_io_vendor_formats.py` 29 pass + 3 conditional skips (agilent test now correctly skips when optional dep absent rather than passing on the wrong contract).
>
> ---
>
> **Earlier 2026-05-08:** **T-CI-1 implementation in flight.** PR #56 opened (`ci/t-ci-1-hygiene-2026-05-08`, head `ee3389e`) addressing 87 of 91 main-branch CI failures across the six in-scope categories from the continuation prompt. CI matrix running. Remaining 4 failures are the codegen/CLI bugs flagged for T-CI-2/3/4. **Three diagnoses corrected mid-execution** (see SESSION_LOG 2026-05-08): jcamp turned out to be a longstanding `io.py` bug exposed by 1.3.0 rename (not pure dependency drift); OPUS/PerkinElmer "missing fixture" diagnosis was actually broken `sys.modules.get` skip-guards using wrong module names; SPC mock needed both buffer-size enlargement AND `NotImplementedError` exception handling. PR description summarizes all corrections.
>
> Local `.venv312` verification before push: jcamp 9/9 (incl. write/read round-trip), SG-derivative 7/7, Optuna callback 1/1, SPC + vendor 40/45 (5 conditional skips), neural_boosted classifier 3/3, trusted regression battery 61/61, all production module imports clean.
>
> **Previous (2026-05-07 late evening):** **T-36 / Item 7 closed: legacy Bayesian path deleted (PR #55 merged → `228bbc2`).** Post-merge investigation surfaced that GitHub Actions CI on `main` has been **red since the workflow was added 2025-10-27** (zero green runs in 6 months — `gh run list --branch main --status success` empty). 91 failing tests on PR #55 baseline; identical to `origin/main`'s pre-merge set (deletion added zero failures). Of the 91: ~73 are headless-Linux Tkinter (xvfb missing in workflow), 5 are `jcamp` library API drift (`jcamp_write` removed upstream), 4 are bad/missing test fixtures, 2 are SG-derivative numerical precision drift (3.86e-12 vs 1e-12 tolerance), 1 is Optuna callback-count drift, 1 is stochastic ML flake, and 4 are real codegen / CLI bugs. CI hygiene filed as **T-CI-1** in `docs/CONTINUATION_PROMPT_2026-05-08_ci_hygiene.md`; real codegen bugs flagged out-of-scope for separate tickets (T-CI-2, T-CI-3, T-CI-4 proposed).
>
