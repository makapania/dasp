...
Fix: `clone(model)` at three pipe-construction sites in `search.py` (`:2191`, `:4161`, `:4163`). Root cause: shared estimator instance being fit first on full preprocessed X (setting `n_features_in_=2151`), then reused in subset-feature fits; sklearn 1.5.2's pre-fit `_check_n_features(reset=False)` fires before the refit would reset the count, raising. sklearn 1.7.2 tolerates it silently (emits `"X does not have valid feature names"` UserWarnings instead).
