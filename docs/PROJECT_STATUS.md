...
> **Last updated:** 2026-04-21 — Mixed-type target Validation-Wide fix merged from `glm/mixed-type-target-validation-wide`. Fixes three sibling crashes (Bayesian validation metrics, stratified split, SPXY split) when classification target column has heterogeneous Python types. 9 GUI sites + 1 search.py site + task-type boundary warning. 151/151 tests pass. Earlier today: `7407140` (one-class export support), `de6b618` (2x2 confusion matrix + NaN-safety for categorical targets), PLS-regression hotfix. First two pushed to `origin/main`, this session's merges pending push.
