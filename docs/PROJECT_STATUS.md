...
> **Last updated:** 2026-05-06 — **Option A Bayesian dedup implemented locally, pending main-thread commits.** Phase 1 was committed by the main thread (`b731d68` MaxTrialsCallback + `a0e6c34` plan doc). Follow-up phases added pre-fit fingerprint dedup to `unified_bayesian.py` at the fully resolved state immediately before CV, with `TrialPruned` re-raised before the broad exception handler, COMPLETE-trial resume rehydration from stored `trial.user_attrs['fingerprint']`, one-class fingerprinting before `run_one_class_cv`, and legacy parity in `bayesian_utils.create_objective_function`.
>
> **Locked decisions honored:** production still uses TPE; A/B harness monkeypatches to `RandomSampler(seed=42)` only. PLS too-many-components still returns `1e10`. PCA-SIMCA's internal `n_components` clamp is documented as an accepted duplicate blind spot rather than mirrored.
>
> **Tests/A-B:** new `tests/test_bayesian_dedup.py` (4 tests) covers fingerprint fields, prune propagation, resume rehydration, and forced duplicate CV count. `tests/test_cv_pls_clamp.py::TestRunBayesianSearchPLSGridClamping` now includes a no-duplicate-fits assertion. A/B harness `tools/ab_dedup_compare.py` ran on `example/` ASD spectra + `BoneCollagen.csv`: PLS regression, LightGBM regression, and LightGBM classification all matched pre-fix rows 100.0% with `pre_unique_count=12`, `post_row_count=12`.
>
> **Verification commands:** `pytest tests/test_bayesian_dedup.py -q --basetemp=/tmp/pytest_codex` (4 passed); `pytest tests/test_cv_pls_clamp.py::TestRunBayesianSearchPLSGridClamping -q --basetemp=/tmp/pytest_codex` (2 passed); `pytest tests/test_unified_bayesian_baseline.py -q --basetemp=/tmp/pytest_codex` (10 passed); `python tools/ab_dedup_compare.py --n-trials 12 --max-features 120` (all three scenarios 100.0% match).
>
