...
> | _(P4)_ | `6bc2b20` | `fix/Tcv-...` (PR #24) | T-20c: PLS-DA parity row + MLP imbalance-is-no-op marker. PLS-DA pins the three-step Pipeline contract (PLS scores → StandardScaler → LogisticRegression) end-to-end; MLP marker pins that `imbalance_method='class_weight'` is a no-op for MLPClassifier (catches the same bug class as T-19's source-inspection tests via numerical predict_proba parity). | GLM cross-family **READY_TO_MERGE** (1 LOW about a pre-existing codegen pattern). DeepSeek review in flight at filing — not yet returned. **PR not yet opened pending DeepSeek's verdict.** |
>
