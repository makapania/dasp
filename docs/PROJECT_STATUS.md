...
> **Last updated:** 2026-04-19 by GLM-5.1 — One-class model configuration parity implemented. Five collapsible hyperparameter cards added to Tab 4C (OneClassSVM, IsolationForest, EllipticEnvelope, LOF, PCA-SIMCA) using old-style BooleanVar + Checkbutton pattern. Backend `_resolve_one_class_model_grids()` helper preserves curated defaults when cards are untouched, uses Cartesian product only for customized models. Bayesian one-class path remains separate and unchanged. 60/60 OC tests pass (54 existing + 6 new `TestOneClassModelConfigParity`). Prior session notes below.
