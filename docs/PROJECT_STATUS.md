...
> **Last updated:** 2026-05-08 (latest) — **T-16 Phase 1 (CV-ANOVA p-value column) shipped on `feat/T16-phase1-cv-anova` at `d4bcca7`, pushed.** Plan at `docs/plans/2026-05-08-T16-phase1-cv-anova.md` (v2 with Codex review folded in: 1 BLOCKER + 3 MEDIUM + 1 LOW all addressed). New `cv_anova_pvalue` column populated for PLS regression rows in BOTH grid path (`search.py:_run_single_config`) and Bayesian path (`unified_bayesian.py` objective + `convert_study_to_dataframe`, via trial user_attr). Per Eriksson, Trygg & Wold 2008. Pure additive — no change to which rows get computed, which row wins, or any search-loop behavior. Numerical impl uses `scipy.stats.f.sf` for tail stability. **PLS-DA explicitly excluded** because dasp's PLS-DA is a `PLSTransformer + LogisticRegression` hybrid (`models.py:1354`), not canonical PLS-on-dummy-Y; CV-ANOVA mathematically requires PLS regression on Y. PLS-DA Q1 coverage will come via Phase 2 permutation test (separate ticket; model-agnostic). Tests: 11/11 pass (`tests/test_cv_anova.py`: 9 unit + 2 integration covering grid + Bayesian). Adjacent regression: 51/51 pass (`test_bayesian_dedup`, `test_t44_autoscale_wiring`, `test_cv_pls_clamp`). Effort: ~half day source + tests, on plan estimate. **NOT MERGED — branch pushed; user opens PRs.**
>
> ---
>
