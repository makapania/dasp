...
> ## ▶ ACTIVE DIRECTION (2026-07-05) — T-31 Multi-Class SIMCA: **Phases A + B + C + D COMPLETE + MERGE-GATE PASSED + "run a selected leaderboard result" follow-up SHIPPED** on `feat/T31-multiclass-simca` (pushed, NOT merged). NEXT = **user greenlight to merge** (do NOT auto-merge) + a live GUI `run`/`screenshot` pass (still owed — verified headless only this session).
