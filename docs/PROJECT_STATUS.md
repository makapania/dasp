...
> ## T-17 IN PROGRESS — 2026-07-01 — **Multi-target regression search** on branch `feat/T17-multitarget-regression` (autonomous overnight build).
> Spec: `docs/plans/2026-07-01-T17-multitarget-regression.md`. Goal: joint multi-output regression (PLS-2 flagship + whole model menu, JOINT vs INDEPENDENT honestly labeled) as a Grid-only superset path (n_targets ≥ 1) that leaves the legacy single-Y path BYTE-IDENTICAL. Build order: foundation `multi_y.py` → PLS-2 JOINT → other JOINT (RF/MLP/CatBoost/XGBoost) → INDEPENDENT (SVR/LightGBM/Lasso/EN/NeuralBoosted) → varsel+VIP → export+model_io. Per-feature gate: tests green + single-Y pins byte-identical + Codex/DeepSeek review + commit. See "Next steps" at bottom / SESSION_LOG for live progress.
>
> **F7-export-modelio DONE (2026-07-01, tag `t17-F7-export-modelio`).** Multi-Y code-export templates (`templates/header.py` DATA_LOADING_MULTITARGET, `templates/validation.py` FoldYScaler + `multi_y_cv_pool`-mirroring CV loop + per-target metrics + final-model blocks) and an isolated `code_generator._generate_multitarget_script()` / `_generate_multitarget_notebook()` path (gated on `is_multitarget = len(target_names) > 1`, so single-Y export stays byte-identical). `model_io.save_model` gains optional `y_scaler=` persisting per-target mean/std to `y_scaler.npz`; `load_model` reconstructs a `FoldYScaler` under `'y_scaler'`; `predict_with_model` inverse-transforms JOINT multi-target preds to RAW units (gated on y_scaler presence). Docstrings updated for the `(n_samples, n_targets)` case. **Tests:** `tests/test_multitarget_export.py` (13 assertions: exported script AND notebook reproduce in-app `run_multitarget_search` pooled preds bit-identically for JOINT PLS / INDEPENDENT Ridge / MOR SVR, joint-Q²/per-target-metric parity, SCALE_Y honesty, single-Y non-activation) + `tests/test_multitarget_save_load_roundtrip.py` (mirrors autoscale roundtrip: 5c(a) reloaded predict bit-identical, 5c(b) Y-scaler stats reload + inverse-transform, 5c(c) target_names/per-target metrics survive in order, RAW-unit RMSE tracks divergent target scales, INDEPENDENT persists no scaler, single-Y byte-identity guard). Full guardrail sweep 234 passed (2 pre-existing env-only `test_export_code` subprocess failures unrelated).
>
> ---
>
