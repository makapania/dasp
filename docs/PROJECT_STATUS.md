...
> ## ▶ BLOCKER (2026-07-07) — T-17 is NOT merge-ready: INDEPENDENT-mode variable selection is secretly COUPLED. Fix before merge.
> User validation on real data (Norway leaf, N+ADF) exposed a genuine bug: in INDEPENDENT coupling mode, CARS/UVE/importance/iPLS run **once on the stacked multi-column Y**, and the single subset is shared by every target's separately-fit model — so a strong target is forced to use variables chosen to also satisfy the other target. Confirmed in source (`multitarget_grid.py:360-362` etc.) AND empirically via the real backend: on N+ADF, CARS-on-N gives N CV R²=0.859 but CARS-on-joint gives 0.712 (byte-matches the real multi-target run); the entire ~0.15 R² gap is the shared subset (preprocessing matched, ranking not the cause). This violates the INDEPENDENT honesty contract (models are per-target, varsel is not) and is why multi-target underperformed single-Y per target. Earlier reviews missed it — the recovered-WIP varsel was checked that it RUNS, never that it SELECTS per-target-appropriate variables (the chemometrics-gate that was skipped). **Decision (user, 2026-07-07): fix before merge; documented now, build tomorrow.** Full ticket + root cause + fix threading + acceptance criteria + reproduction: **[CONTINUATION_PROMPT_2026-07-08_T17-independent-varsel-fix.md](CONTINUATION_PROMPT_2026-07-08_T17-independent-varsel-fix.md)**. Fix = per-target varsel under INDEPENDENT coupling (JOINT keeps joint varsel); threads per-target variable indices through cell build/eval/refit/save/GUI. Until fixed, do NOT merge T-17 / PR #63. (Everything else below — the UVE/CARS landing + the 3-wave GUI integration — is done and green; this is the one blocker.)
>
> ## ACTIVE DIRECTION (2026-07-07 later) — T-17 multi-target GUI INTEGRATION complete (3 waves). Feature now behaves like the rest of the program.
> User smoke-tested the multi-target tab and found it poorly integrated ("format unlike all other methods; nothing happens in progress; results stay in the same tab; can't double-click to run a model; can models be saved/exported?"). Root cause: the tab was built as a self-contained island. Fixed additively across 3 waves (Fable orchestrator drove GLM-5.2 workers; Opus for the hard/shared-path items + git + gates), zero regression to single-Y/classification. **Wave 1 (`3e9f092`):** inner sub-notebook (Setup/Progress/Results); live progress surface (bar/ETA/best-model/capped log) consuming the callbacks the poller used to discard; backend now emits progress during the previously-silent varsel first pass; Pause/Resume/Stop; disk log. **Wave 2 (`35cdcfc`):** leaderboard parity (Rank col, header tooltips, themed listbox, stable iids, export-lock-while-running); **double-click → detail dialog** (per-target predicted-vs-actual plots, metrics, correlation panel, 300-DPI export; OO-Figure not pyplot — an Opus-caught intermittent TclError); **Single/Multi toggle in the main Results tab** surfacing MT results there via an additive mirror tree (single-Y widgets pack/unpack-hidden, never re-rendered). Also sortable columns (`9222e63`). **Wave 3 (`ea01936`):** `refit_multitarget_final` (replays preprocess→varsel→fit on full calibration to reconstruct a fitted model from a result; JOINT y-scaler persisted; save→reload→predict exact 0.00e+00); **Save Model (.dasp)** + **Export Code** (script+Colab notebook via is_multitarget CodeGenerator) wired in the detail dialog; **Prediction tab** fans multi-target `(n,k)` predictions into per-target columns (single-Y/one-class path byte-identical). **QA:** two layers every wave — my verification gate + Fable adversarial (refit exact-roundtrip, tag-collision defused, 102/102 exact n_variables, single-Y byte-identity). model_io UNCHANGED (single-Y save/load byte-identical by construction). **Deferred (own ticket, user decision):** interactive Model-Dev multi-target refinement (double-click opens the read-only detail dialog for now). **Final full-suite gate DONE (2026-07-07):** 6 failed / 3020 passed / 33 skipped — the 6 = the same pre-existing set that fails on `main` (5 documented codegen/CLI + `test_catboost_via_gui` env), **ZERO new failures from the integration** (+28 net passing vs pre-integration baseline). **Remaining before merge:** user GUI re-smoke of the new progress/detail/save/export/predict flow + push greenlight for PR #63.
>
> ## ACTIVE DIRECTION (2026-07-07) — T-17 UVE/CARS multi-Y SHIPPED (supersedes the "v1.1 deferral"). Landing branch fit-for-purpose.
> The overnight-recovered multi-target UVE + CARS-Tree varsel WIP (`bc750d9`) is now **completed to "ship, not revert"** and landed on `feat/T17-multitarget-regression` (`ad92cc1` Wave 1 backend+tests, `9841d0f` Wave 2 GUI). **This supersedes every "UVE/CARS on multi-Y = v1.1 deferred" / "raise NotImplementedError on 2-D Y" note in the older blocks below** — UVE/CARS + hybrids (`uve`, `cars`, `uve_cars`, `uve_spa`, `uve_cars_spa`, `fipls_cars`) and legacy `ipls` now support 2-D Y via the importance route in `multitarget_grid.py`. **What landed:** (1) SPA preflight now gated — `verify_spa_multi_y_safe` runs only when an SPA-dependent method is selected (was unconditional, wasting time on every run — Codex blocker); (2) UVE cutoff/components threaded **GUI→backend end-to-end** (`_collect_multitarget_config` → `run_multitarget_grid_search` → `build_multitarget_varsel_subsets` → `_model_independent_importances` + `get_uve_threshold`), closing the "UVE GUI controls ignored" Codex blocker; (3) stale reject/skip test contract flipped to **positive multi-Y support** + new `TestMultiYSupport`/`TestMultiYUveCarsSupport` coverage; (4) integration test corrected to **Independent-only default + a `coupling_mode="both"` companion**. **Decisions (see `docs/T17_LANDING_BLUEPRINT.md`):** vcpa-iriv stays **skip-with-notice** (unrouted; its leaf's 2-D PLS branch is not wired into the grid); `ga_lightgbm` multi-Y fitness documented as **intentionally-unrouted** (grid routes linear-GA only; the code is live in single-Y `search.py`); the phantom "T-17 cost notice" comments **removed**. Only multi-target **classification** CARS remains rejected (no coherent joint PLS-DA criterion). **Gate:** 294 passed / 0 failed across the 10-file multitarget+varsel suite. **Full-suite failure-diff vs `origin/main` DONE (2026-07-07): 6 failed / 2992 passed / 33 skipped — the 6 = the 5 documented main-red codegen/CLI tests (`test_cv_strategy` nameerror, `test_export_code` ×2, `test_t19` ×2) PLUS `tests/gui/test_comprehensive.py::test_catboost_via_gui`, all of which ALSO fail identically on `main` (efad2f6) → ZERO new failures from T-17.** (The catboost failure is a pre-existing environmental bug: catboost 1.2.10 is installed but the model registry reports it unavailable — separate follow-up, not a T-17 blocker; the baseline was previously miscounted as 5.) **Remaining merge-gate step: manual GUI smoke** (load example ASD + BoneCollagen, 2 targets, UVE+CARS with non-default cutoff, confirm UVE/CARS cells land on the leaderboard). **Why now:** landing T-17 fit-for-purpose is the prerequisite for the next feature, **VIP-CARS** (design note `docs/METHOD_DESIGN_vip_cars.md` + plan `docs/IMPLEMENTATION_PLAN_vip_cars.md` on `main`), which depends on this multi-target CARS machinery. Execution: Opus orchestrator drove 5 GLM-5.2 write-mode workers across 2 file-disjoint waves + a Fable triage/blueprint pass. See SESSION_LOG 2026-07-07.
>
> ## ACTIVE DIRECTION (2026-07-04) — T-17 coupling-mode-choice feature DONE; whole unpushed stack about to push to PR #63.
> **Coupling-mode-choice feature is DONE** (`8547b9c`/`7c857ed`/`4b87be2` + cell-count fix `b509fc4`). Coupling is now a user choice — **Independent / Joint / Both, default = Independent (user-confirmed 2026-07-04)**. INDEPENDENT is available for ALL models (PLS-1-per-target etc.); JOINT where a variant exists; no-joint-variant models in Joint mode raise the catchable `NoJointVariantError` → skip-with-notice. **Codex-reviewed clean** (1 LOW: pre-run cell-count over-estimate — the flat `n_cfg×2`/`×1` ignored joint capability; now capability-weighted per model via `_multitarget_cell_lower_bound`, mirroring backend `_expand_model_modes` exactly, with a discriminating test). **216 tests green.** `search.py` **0-diff** (single-Y byte-identity intact); `resolve_multitarget_strategy(mode=None)` keeps the table default so the F2 seed evaluator + single-Y consolidation pin stay byte-identical. One intended behavior change: a DEFAULT multi-target grid run now produces INDEPENDENT (PLS-1) rather than the prior JOINT (PLS-2). **The whole unpushed stack (5 UX fixes + coupling + cell-count fix) is about to be pushed to PR #63** (the main session pushes). Full details: SESSION_LOG 2026-07-04.
>
> ## ACTIVE DIRECTION (2026-07-04) — T-17 layering post-review UX fixes + a coupling-mode follow-up onto the merge-ready branch (PR #63 pushed, NOT merged).
> After the build was merge-ready (below), user hands-on testing found 5 GUI issues — all fixed on the branch (`a4fe20d`..`fc6e33e`, 109 tests green): (1) target list now refreshes on tab-show; (2) results horizontal scroll fixed (`content_frame` column weight + `stretch=False` columns — verified visually); (3) compact preprocessing-detail label (`snv_deriv d2 w17 p3`); (4) effective-N notice for the complete-case shrink; (5) SPXY/Stratified greyed out for multi-target (single-Y only) → Kennard-Stone default. **NEXT (queued): coupling-mode-choice feature** — make coupling a user selection (Independent / Joint / Both; INDEPENDENT for ALL models incl. PLS-1-per-target, JOINT where supported, "Both" for on-leaderboard comparison; default Independent). Spec: `docs/superpowers/specs/2026-07-04-multitarget-coupling-mode-choice.md`; Opus orchestrator + GLM-5.2 workers. These UX + coupling commits are NOT yet pushed to PR #63 (the earlier 05ce513 build IS pushed). Full details: SESSION_LOG 2026-07-04.
>
> ## (build baseline) T-17 "FULL SEARCH PARITY" — BUILD COMPLETE (Phases A+B+C), full review + merge-gate DONE. PR #63 (pushed at 05ce513), awaiting user greenlight.
> All 3 phases shipped on `feat/T17-multitarget-regression`: **Phase A** backend `multitarget_grid.py` (`4d88607`..`86a75fa` + fixes `1b1518f`..`913cda6`), **Phase B** GUI (`42cb50b`/`5e46650` + fixes `4cdb47a`..`898e083`), **Phase C** integration (`4210b47`), **final toolkit fixes** (`5aaccf9`..`246c70e`). **Review trail:** Codex + Kimi K2.7 per phase (Phase A: 8 defects incl 2 CRITICAL; Phase B: 6 defects incl 2 HIGH) + full pr-review-toolkit at merge-readiness (code-reviewer CLEAN; silent-failure-hunter + pr-test-analyzer found `apply_uve_prefilter` half-wired, an importance-array `argsort` NaN trap, a varsel-abort asymmetry, a tautological test) — all fixed with discriminating tests (revert-and-confirm-FAIL verified). **Merge gate:** full suite ex-GUI at Phase C = **5 failed / 2850 passed / 26 skipped**, the 5 = the documented pre-existing main-red codegen/CLI set (test_cv_strategy nameerror, test_export_code ×2, test_t19 ×2) → **zero new failures vs origin/main**. **Final post-fix re-run CONFIRMED: 5 failed / 2864 passed / 26 skipped (identical 5-failure set; +63 net passing over the session).** Real-data e2e: 120 cells, jointQ2 0.9683 (vs 0.71 single-config foundation), round-trip max|diff|=0. `run_search` single-Y path 0-diff (byte-identical). **Deferred (non-blocking):** HP-text `except: pass` swallow (mirrors single-Y — fixing only MT diverges), Black (repo-wide), `best_model` live-update + Cancel-label polish (enhancements). **DO NOT auto-merge PR #63 — awaiting explicit user greenlight.** Execution model: Opus orchestrator drove GLM-5.2 workers per task/fix; Opus reviewed at each phase boundary + merge-readiness. See SESSION_LOG 2026-07-02 (Phase A / Phase B / merge-readiness).
>
> ## (superseded) T-17 — Phases A (backend) + B (GUI) COMPLETE + REVIEWED + FIXED; Phase C (integration) NEXT.
> **Phase A done:** new `src/spectral_predict/multitarget_grid.py` orchestrator (preprocessing × varsel × model × hp grid, joint multi-Y CV, NaN-safe rank), Tasks 1-11 (`4d88607`..`86a75fa`); Codex+Kimi review caught **8 real defects** (2 CRITICAL: varsel cache-key collision + `fipls_spa` never wired into importance adapter), fixed (`1b1518f`..`913cda6`). SPA verified 2-D-safe → `spa`+`fipls_spa` IN.
> **Phase B done:** GUI tab reworked (Tasks 12-13, `42cb50b`/`5e46650`) — inherits full normal-Run config (incl. all 11-model HP overrides), dispatches `run_multitarget_grid_search` on a worker thread via a thread-safe **queue + main-thread `root.after` poller** (not `root.after` from the worker — Tcl rejects that on Win32), separate `_multitarget_controller` + Cancel, leaderboard columns + skip-notice + CSV. Codex+Kimi review caught **6 defects** (2 HIGH: repeat-run orphans worker/poller; terminal-event race stuck on "Running…"), all fixed with discriminating tests (`4cdb47a`..`898e083`). One backend touch: `SearchController.is_ended` @property→method (all call sites migrated). **23 GUI/controller tests pass; single-Y `run_search`/`search.py` = 0 diff (byte-identity intact).**
> **NEXT: Phase C = plan Task 14** (end-to-end integration on real ASD spectra `read_asd_dir('example')` + constructed 2nd target; JOINT/INDEPENDENT/varsel cells; save→reload→predict round-trip). **THEN merge-readiness: full pr-review-toolkit + diff-failure-set gate vs `origin/main`.** Execution: Opus orchestrator drives GLM-5.2 workers (or edits directly when overlapping/CRLF-risky); Opus reviews at each phase boundary. Do NOT merge PR #63.
>
> ## (superseded) T-17 "FULL SEARCH PARITY" — spec + plan committed; IMPLEMENTING Phase A (GLM-5.2 workers, Opus orchestration).
> Give the multi-target path full parity with the single-Y search (grid over **preprocessing × varsel × model × hyperparameters**, joint multi-Y CV, ranked by joint Q², real leaderboard). **Spec `docs/superpowers/specs/2026-07-02-multitarget-full-search-parity-design.md` (v3, `9449420`)** — review-hardened via 5 cross-family passes (Codex ×2, Kimi K2.7, MiMo 2.5, GLM 5.2). **Plan `docs/superpowers/plans/2026-07-02-multitarget-full-search-parity.md` (`25d8141`)** — 14 TDD tasks (Phase A backend T1-11, Phase B GUI T12-13, Phase C integration T14). RESOLVED decisions: D1 sequential v1 (in-cell model n_jobs); D2 skip 1-D preprocessing-discovery + basic-grid fallback; D3 own test-pinned enumerator; HP inheritance (a) extend JOINT builders to full parity (INDEPENDENT + XGBoost already full). Varsel (verified 18-method inventory): 4 interval + spa/fipls_spa (gated on spa 2-D verification) + ga-linear + importance supported; UVE/CARS (approved deferral) + ipls-legacy/vcpa-iriv skip-with-notice. NEW file `src/spectral_predict/multitarget_grid.py` is the orchestrator; `run_search` UNTOUCHED. **Execution model: Opus orchestrator subagent drives GLM-5.2 write-mode workers per task; Opus reviews at each phase boundary (Codex + cross-family + toolkit + merge-gate vs origin/main).** Do NOT merge PR #63. Handoff: `docs/CONTINUATION_PROMPT_2026-07-02_T17-full-parity.md`. GUI bug fixes already landed (`52c814b`).
>
> ## T-17 MERGE-READY — PR #63 OPEN (do NOT auto-merge) — 2026-07-02 — Multi-target regression on `feat/T17-multitarget-regression`, tip `051151b`. Pushed. **PR #63 → `main` open; awaiting user greenlight.**
> **Update — full pr-review-toolkit round (5 Claude-family specialists) after the 4 cross-family passes:** the two layers are orthogonal — the toolkit found a HIGH the cross-family passes missed. **silent-failure-hunter + code-reviewer + pr-test-analyzer independently converged** on a THIRD instance of the NaN→fake-perfect sink: `ga_pls._fitness_function` multi-Y branch (a broken chromosome scored `-0.0` = MAX fitness since GA maximizes → would WIN the wavelength set). Fixed `051151b`. Also folded: `model_io.save_model` now raises if a JOINT multi-target model is saved without its y_scaler (silent scaled-units trap); two doc-accuracy fixes (`reduce_multi_y_score` zero-caller rot, `extract_pls_multi_y` >=1.3→>=1.1); three coverage pins (FoldYScaler train-only no-leakage — the central invariant; repeated_kfold pooling branch; native-Ridge INDEPENDENT equivalence — investigation confirmed NO coupling bug, 2-D solve == N Ridge fits to ~1e-15). **Deferred (defensive/polish, non-blocking, in PR comment):** type enforcement-altitude (`MultiTargetStrategy.__post_init__`, frozen-dict sharing, `MultiTargetResult` shape checks, `FoldYScaler` width-1 broadcast); `inter_target_correlation` constant-target nan advisory; GUI synchronous multi-target search (errors surface loudly via messagebox); interval-subset Returns multi-Y docstring note. 168 multi-target/varsel/vip/ga_pls/save-load + 74 model_io tests green.
> **The NaN-sink pattern had 3 sites found across 3 review rounds (GLM 5.1 → GLM 5.2 → toolkit).** Lesson in SESSION_LOG: a fix-of-a-fix demands a whole-repo grep for the exact idiom (`mean_nmse > 0.0 else 0.0`) — all three would have surfaced at once.
> **MERGE GATE PASSED (2026-07-02, local diff-failure-set, Windows Py3.12 `.venv312`, `--ignore=tests/gui`):** ran the full suite on HEAD `4d5be73` (5 failed / 2801 passed / 26 skipped) and on `origin/main` `ad12722` (5 failed / 2673 passed — branch adds ~128 tests). Failure sets are **IDENTICAL** → **zero new failures introduced**. The 5 are the known pre-existing main-red set (T-CI-3/T-CI-4: `test_cv_strategy` nameerror, `test_export_code` ×2, `test_t19_class_weight_per_library` ×2 — all codegen/CLI). Baseline run via in-place detached `origin/main` checkout, branch restored clean afterward. **PR #63 is clear to merge on user greenlight** (per process: still awaiting explicit greenlight; do NOT auto-merge).
>
> Multi-target regression on `feat/T17-multitarget-regression`. Pushed. **PR #63 → `main` open; awaiting user greenlight.**
> This session took the F1–F7 build to merge readiness (no new features):
> - **Test verification (`.venv312`):** the F1–F7 suites re-run green — 156 passed (138 core + 18 GUI/VIP pins).
> - **End-to-end smoke on REAL 49×2151 ASD spectra** (BoneCollagen %Collagen + a 2nd spectrally-grounded target, corr 0.83): `run_multitarget_search` (grid) ranked/labeled correctly; grid-only guard rejected Bayesian & NSGA-II; fit JOINT PLS-2 → `save_model(y_scaler=)` → `load_model` → `predict_with_model` reproduced in-app RAW-unit preds at **max|diff| = 0.0**. (Real per-target: Ridge-INDEP jointQ²=0.82 Collagen R²=0.84 RPD 2.5; PLS-2 JOINT jointQ²=0.71.)
> - **Whole-diff cross-family review (4 passes, all SAFE_TO_MERGE after fold-in):** Codex (call-graph/force-to-grid — clean), Kimi K2.6 (single-Y byte-identity sweep — no regressions, ran gold suite), GLM 5.1 (multi_y math — invariants verified), **GLM 5.2** (final-diff, upgraded per user — confirmed prior + caught one new MEDIUM). **Three MEDIUMs folded in:** (1) `5ab269a` NaN-safe multi-target ranking in `run_multitarget_search` (non-finite joint_q2 no longer sorts to best); (2) `5ab269a` pin for the byte-identical `compute_vip` `Q.ndim==1` branch; (3) `283bfce` NaN-safe `rmsecv` sentinel in `_evaluate_interval_pls_multi` (a non-finite per-target q2 collapsed to a spurious `rmsecv=0.0` that would WIN iPLS/MC-siPLS/MWPLS selection → now falls to the `(inf,-1.0)` worst-sentinel). Each pinned by a discriminating regression test.
> - **Deferred (LOW, practically-unreachable):** RPD/RER `else 0.0` sentinel at exactly RMSE==0 (`multi_y.py:319`); `_prep_varsel_y` 0-D scalar parity; constant-target joint_q2 inflation (informational).
> - **Merge gate (unchanged):** `main` red on cloud CI since 2025-10-27 → gate is local diff-failure-set (PR adds zero new failures vs `origin/main`), NOT the CI rollup. Run before merge, on greenlight.
> - **Tooling aside:** fixed stale `glm-5.1` defaults in `~/.claude/agents/opencode-call.md` (its alias table already said 5.2); added memory `feedback_model_version_aliases` — dispatch by alias (`glm`→5.2, `kimi27` for code tasks), never hardcode a version.
> - **v1.1 deferrals (by design):** UVE/CARS on multi-Y; Bayesian/NSGA-II multi-target (grid-only, 3 layers); F6 per-model hyperparameter grids; booster early-stopping under multi-Y.

> ## T-17 BUILD COMPLETE (F1–F7 all shipped) — 2026-07-01 — **Multi-target regression search** on branch `feat/T17-multitarget-regression` (autonomous overnight build). Tip commit `8c66f1c`. Not yet merged; not yet on remote until this session's push.
> Spec: `docs/plans/2026-07-01-T17-multitarget-regression.md` (canonical spec: `~/.claude/plans/i-want-you-to-humble-crystal.md`). Goal delivered: joint multi-output regression (PLS-2 flagship + whole model menu, JOINT vs INDEPENDENT honestly labeled) as a Grid-only superset path (n_targets ≥ 1) that leaves the legacy single-Y path **BYTE-IDENTICAL** (proven by gold fixtures). Continuation: `docs/CONTINUATION_PROMPT_2026-07-02_T17.md`.
>
> **All 7 features done, tested green, per-feature reviewed (Codex + DeepSeek), each committed + tagged `t17-F{n}-*`:**
> - **F1-foundation** (`95f1437`, review `ebb31b3`) — `src/spectral_predict/multi_y.py`: `FoldYScaler` (JOINT-only per-target train-fold Y autoscaling + inverse), `multi_y_cv_pool` (shape-aware (n,n_targets) CV pooling reusing `build_cv_splitter`, per-fold clone+scale, inverse to RAW before pooling; `scale_y=False` raw INDEPENDENT path), `multi_y_metrics` (RAW-unit per-target R²/RMSE/Q²/RPD/RER/CCCcv/Bias; joint Q² = mean per-target Q²), `extract_pls_multi_y` (coef_.T → (n_features,n_targets)), `cap_components`. 22 tests.
> - **F2-orchestrator-pls2** (`c1de0f3`, review `7298f46`) — `src/spectral_predict/multitarget_search.py`: `resolve_multitarget_strategy` (per-model JOINT/INDEPENDENT resolver, UNKNOWN raises, verbatim INDEPENDENT precise-note), `build_multitarget_estimator` (PLS-2 wired scale=False + cap_components), `run_multitarget_search` (per-target RAW metrics, joint-Q² ranking, correlation guardrail, hard grid-engine-only guard rejecting Bayesian/NSGA-II).
> - **F3-joint-models** (`c272d96`) — RF/MLP/CatBoost(MultiRMSE)/XGBoost(multi_output_tree) native JOINT builders; booster early-stopping disabled under multi-Y for v1.
> - **F4-independent-models** (`c431849`) — Ridge native 2-D solve; SVR/LightGBM/Lasso/ElasticNet/NeuralBoosted via `MultiOutputRegressor`; optional JOINT MultiTaskLasso/MultiTaskElasticNet. INDEPENDENT-at-fixed-config == N separate single-target fits bit-for-bit.
> - **F5-varsel-vip** (`5a3f637`, review `13cc81e`) — iPLS fwd/bwd + MC-siPLS + MWPLS + SPA + GA-PLS + `compute_vip` extended to multi-Y via additive `if y.ndim==1` (exact legacy) / `else` branches; single-Y proven byte-identical by `tests/gold_standards/varsel_single_y.npz` + `TestSingleYByteIdentityGold`. UVE/CARS + hybrids raise `NotImplementedError` on 2-D Y (v1.1 deferred).
> - **F6-gui** (`02ca8f1`, review `10c1db2`) — new "Multi-Target" sub-tab under Analysis Configuration: EXTENDED-select target Listbox, per-model JOINT/INDEPENDENT tags, results Treeview + CSV export with per-target metrics; when >1 target selected the Bayesian/NSGA-II radios grey out and `optimization_method` is forced to `'grid'` (with `run_multitarget_search` backstop). `self.selected_targets` added beside `self.target_column` so single-Y UI is untouched.
> - **F7-export-modelio** (`c338bea`, review `8c66f1c`) — Multi-Y code-export templates + isolated `code_generator._generate_multitarget_script()/_notebook()` (gated on `is_multitarget = len(target_names) > 1`); `model_io.save_model(y_scaler=)` persists per-target mean/std to `y_scaler.npz`; `load_model`/`predict_with_model` inverse-transform JOINT preds to RAW units. Exported scripts AND notebooks reproduce in-app pooled preds bit-identically (max|diff|==0.0).
>
> **Verification (this session, `.venv312`):** `tests/test_multi_y.py tests/test_multitarget_search.py tests/test_variable_selection.py tests/test_multitarget_export.py tests/test_multitarget_save_load_roundtrip.py` → **138 passed**; `tests/gui/test_multitarget_tab.py tests/test_vip_formula.py tests/test_plsda_importance.py` (GUI tab + single-Y VIP pins) → **18 passed**. Pre-existing `test_cv_strategy.py` failure is main-red/unrelated (confirmed at F1 with changes stashed); the two `test_export_code` env-only subprocess failures are unrelated (bare `python` on PATH lacks sklearn).
>
> **Next steps:** open PR from `feat/T17-multitarget-regression` → `main` (do NOT auto-merge; per project process wait for user greenlight and check CI diff-failure-set vs `origin/main` since main has been red since 2025-10-27). Parked blockers: (a) each Codex per-feature review returned NEEDS-CHANGES on F1/F2/F5/F6/F7 and DeepSeek NEEDS-CHANGES on F5/F6 — those were per-feature gate reviews folded into the `fix(T17): review on …` commits already in the chain; a fresh whole-diff cross-family pass before merge is advisable. (b) UVE/CARS multi-Y + Bayesian/NSGA-II multi-target are explicit v1.1 deferrals. (c) F6 per-model configs use default hyperparameters (no full grid) — within scope, notable for follow-up. Details in `docs/CONTINUATION_PROMPT_2026-07-02_T17.md`.
>
> ---
>
