...
> **Phase-B multi-family gate (Codex 5.5 + Kimi K2.7 + MiniMax M3, rotated off GLM):** no BLOCKER; core Wold math + leakage sound. Fixed with red-first tests: below-floor-class varsel crash (`KFold(1)`), empty/invalid `n_select`, non-finite importances, Wold PCA `random_state`. Pushed back (verified) on two MiniMax HIGHs — DPOW RMS-vs-std (probe refutes the std claim: std→ρ≈0, RMS→ρ≈0.93; kept RMS) and own/cross asymmetry (spec-pinned cross-fit; kept). **Open methodology to confirm with user:** (a) DPOW RMS-about-zero convention (recommend keep); (b) `balanced=MPOW·DPOW` combination — normalize or not (deferred); (c) MPOW `ddof` small-n upward bias (documented, refit deferred); (d) supervised-varsel can miss a novel class distinctive only on discarded low-importance features (representative-case guard passes; adversarial case is a documented limitation); (e) model-layer supervised ships `importance` only — the precomputed-mask hook is the extensibility path for C.
> **NEXT: Phase C** (search + task-type wiring: `run_multiclass_simca_search`, `scoring`/`unified_bayesian`/registry branch sites + fall-through audit, result columns, real-data e2e / LOCO on held-out sites) → Phase D (GUI + export). Do NOT merge until Phases C–D + merge-gate. Full detail: SESSION_LOG 2026-07-04 (Phase A + Phase B entries).
>
> ---
>
