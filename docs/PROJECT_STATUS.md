...
> **Last updated:** 2026-04-21 — Imbalance-handling silent-fallback fixes merged (commits `a6955c5`, `b8a70cb`, `f8ab20b`, `ac43289`). Task-type-aware dropdown (classification vs regression methods), one-class imbalance UI hidden, substitution-warning banner in GUI, and `print()` → `logger.warning()`+`progress_callback` in unified_bayesian.py/nsga2_search.py. Codex-flagged banner-ordering bug fixed as `ac43289` (description-update had to run before banner-set). Earlier today: Mixed-type target Validation-Wide fix, `7407140` (one-class export support), `de6b618` (2x2 confusion matrix + NaN-safety for categorical targets), PLS-regression hotfix. Pending push.
