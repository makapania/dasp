...
> ## ▶ ACTIVE DIRECTION (2026-07-29) — Agent-facing API: **CLI retired**, `docs/AGENT_COMPOSITION.md` added, `multiclass_varsel_mask` made public (branch `feat/agent-composition-guide`)
...
>
> **Verification:** 53 contract tests green; every doc example re-executed after each review round. Non-GUI suite (2885 tests) is the correct gate for this change — GUI tests were deliberately skipped because nothing here touches GUI code, and backgrounding them spawns Tk windows whose manual closure kills the run (killed one at 36%, exit 127). One PRE-EXISTING unrelated failure: `test_cv_strategy.py::TestPostMergeReviewFixes::test_classification_metrics_template_has_no_nameerror` (`NameError: _fit_fold`) — verified identical on `main`.
>
