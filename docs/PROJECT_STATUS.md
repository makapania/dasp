...
> **Last updated:** 2026-05-05 morning ticket-closing run — 5 PRs opened (#32–#36) closing the deferred backlog from the 2026-05-04 batch. PR #32 fully cleared by 3 reviewers (READY_TO_MERGE); PR #33 had a sister-site fix-of-fixes after GLM + Codex + DeepSeek convergent HIGH on `unified_bayesian.PIPELINE_PARAMS`; PRs #34–#36 are small / user-approved (no review needed). All branches green on the consolidated 13-file regression sweep.
>
> ## Session 2026-05-05 morning — 5 PRs opened (NOT MERGED — user opens/merges)
>
> | PR | Branch | What it ships | Reviews |
> |---|---|---|---|
> | #36 | `fix/Tgui-auto-resume-tooltip-speed-wording` | P0 user ask: refresh Bayesian crash-resume tooltip — drop stale T-41 planning-era speed claims (`PLS 8x slower`, `XGBoost 1.4x slower`, `1.2x at threshold`) that became wrong after T-42's per-trial-write hoist. Replace with honest framing about per-trial write cost dominating sub-1s trials and `~1.06x of in-memory in benchmarks`. Adjacent stale code comment in `unified_bayesian.py:2232` updated. Wording approved by user mid-session. | None — user-approved wording |
> | #32 | `fix/T20-imbalanced-auto-parity-row` | P1a: closes deferred Codex MEDIUM from PR #22. Adds `test_xgboost_binary_classification_parity_auto_with_correction` mirroring the explicit class_weight row but with `imbalance_method='auto'` and `expect_stdout_marker='applying class_weight'`. Pins the most-leverage T-19 path (auto-resolves-to-class_weight) that previously had no parity coverage. | DeepSeek + GLM cross-family + pr-test-analyzer — ALL READY_TO_MERGE / Approve, zero HIGH findings. One MEDIUM follow-up flagged: LightGBM/CatBoost auto-with-correction rows missing for symmetry (deferred). |
> | #33 | `fix/T20-pipeline-params-strip-bare-njobs` | P1b: closes deferred Codex MEDIUM from PR #22. `_PIPELINE_PARAMS` listed `n_jobs` as a sklearn Pipeline kwarg and stripped it; downstream `setdefault('n_jobs', -1)` then re-injected the parallel default, silently overriding user-set determinism (`n_jobs=1`). `n_jobs` is NOT a sklearn Pipeline kwarg. Fix is one-line removal from the strip set. **Fix-of-fixes** (`b3c5994`): same bug class on Bayesian sister site `unified_bayesian.PIPELINE_PARAMS:88`, used by `_capture_serializable_params` to strip params from Optuna trials. Plus 2 structural pin tests in `test_t20_saved_model_export_parity.py`. KNOWN ISSUE comment added for `verbose` (same architectural smell, deferred). | DeepSeek + GLM + Codex cross-family. **Convergent HIGH** from all 3 reviewers on the sister-site bug (Bayesian-captured params losing user-set `n_jobs`); fix-of-fixes pushed. DeepSeek also flagged a separate observation: `model_class.startswith('LGBM')` at `code_generator.py:988` may be dead code since the resolved class name is `LightGBM`/`LightGBMClassifier`. After this PR's fix the impact is zero (n_jobs survives the strip anyway), so deferred as follow-up. |
> | #34 | `fix/Tridge-classifier-dead-code-delete` | P1c: deletes unreachable `RidgeClassifier` branch at `models.py:471-473`. No entry point can select Ridge for classification (registry / tier / GUI / `is_valid_model` all exclude it). Defensive scaffolding for a feature that never shipped. Closes the P2 investigation filed 2026-05-04 overnight. | None — 4-line pure deletion |
> | #35 | `fix/Tplsda-ndim-fallback-codegen-port` | P1d: closes deferred DeepSeek M2 from PR #28. Ports the `ndim>2` fallback from `models.PLSTransformer.transform` (lines 66-68) to the codegen's inline `PLSTransformer.transform`. Currently unreachable (PLS-DA always fits 1D y); future-proofs against T-17 (PLS-2 multi-target, ~2-3w out). | None — 2-line defensive port |
>
> **Test counts**:
> - Pre-session baseline (consolidated 13-file regression sweep on `origin/main` = `0576358`): 333 passed + 1 skipped.
> - PR #32 alone: 333 + 1 skipped (test row added, included in baseline math).
> - PR #33 with fix-of-fixes: 354 passed + 1 skipped (full T-20 parity matrix [21 = 19 original + auto-with-correction + 2 structural pin tests] + consolidated sweep).
> - PR #34, #35, #36 individually: 333 + 1 skipped (no test additions).
>
> **Branch state at session-end**: 6 branches pushed to `origin` (the 5 PRs above plus the abandoned `fix/Tgui-auto-mode-speed-wording` from earlier in the session — that branch's wording bundle was lost when `origin/main` advanced to `0576358` post-fix-of-fixes; PR #36 re-applies the same bytes from a fresh branch). Local `main` was diverged from `origin/main` at one point (had unpushed commit `0bb7e19` containing the bundled wording fix) — re-fetch corrected this; the working state of `origin/main` is authoritative.
>
> **Process lessons captured this session** (full writeups in SESSION_LOG):
> 1. **Parallel-session collision is real**: `origin/main` advanced from `b0d031f` → `0576358` (5 commits) during my investigation. Local `main` was tracking the OLDER state. Re-fetch caught this; without it, branches off "main" would have included unrelated commits in their PR diffs. Mitigation: always re-fetch and branch off `origin/main` (not local `main`) when work spans long agent-running periods.
> 2. **Edit tool CRLF/LF normalization can introduce phantom whitespace hunks**: an Edit on `models.py` for the Ridge deletion produced an unintended whitespace-only hunk at lines 1807-1817 because the file had mixed CRLF/LF endings and the Edit tool's write normalized some lines. Workaround: use a binary-mode Python script (`Path.read_bytes()` → `.replace()` → `Path.write_bytes()`) for surgical edits when line endings are mixed.
> 3. **Cross-family + Codex convergent on sister sites is the strongest signal**: GLM, Codex, AND DeepSeek all independently flagged the `unified_bayesian.PIPELINE_PARAMS` sister site as HIGH on PR #33. Three orthogonal reviewers converging on the same finding is essentially proof of the bug class. The codex-round-2 lesson from PR #24 (call-graph reasoning catches sister sites) reaffirmed: even the cross-family pair caught it this time, possibly because GLM's grep sweep was extensive. Codex's specific value is when the sister site is mediated by a non-obvious dispatcher branch.
>
> ---
>
