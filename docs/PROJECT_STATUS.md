...
- **Crash-resume radio flip removed (branch `fix/resume-no-persistence-flip`, PR open).** Premise verified with a real `os._exit` crash after migration: 'auto' resumes the saved study (`tests/test_resume_under_auto_after_crash.py`). GUI resume no longer sets persistence to 'always'; the prompt is skipped when no SQLite store exists ('never', or 'auto' crashed in warmup). Legacy unfingerprinted studies are not resumed under 'auto' (stay in memory, untouched) — accepted.
