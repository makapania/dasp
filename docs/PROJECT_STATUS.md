...
> **Last updated:** 2026-05-07 late evening — **T-36 / Item 7 closed: legacy Bayesian path deleted (PR #55 merged → `228bbc2`).** Post-merge investigation surfaced that GitHub Actions CI on `main` has been **red since the workflow was added 2025-10-27** (zero green runs in 6 months — `gh run list --branch main --status success` empty). 91 failing tests on PR #55 baseline; identical to `origin/main`'s pre-merge set (deletion added zero failures). Of the 91: ~73 are headless-Linux Tkinter (xvfb missing in workflow), 5 are `jcamp` library API drift (`jcamp_write` removed upstream), 4 are bad/missing test fixtures, 2 are SG-derivative numerical precision drift (3.86e-12 vs 1e-12 tolerance), 1 is Optuna callback-count drift, 1 is stochastic ML flake, and 4 are real codegen / CLI bugs. CI hygiene filed as **T-CI-1** in `docs/CONTINUATION_PROMPT_2026-05-08_ci_hygiene.md`; real codegen bugs flagged out-of-scope for separate tickets (T-CI-2, T-CI-3, T-CI-4 proposed).
>
