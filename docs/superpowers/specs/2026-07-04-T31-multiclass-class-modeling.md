...
- **Score convention — normalize to "higher = more normal" per engine (MiniMax — must pin or IF inverts):** `OneClassSVM.decision_function` (≥0 inlier, use as-is), `EllipticEnvelope.decision_function` / `LocalOutlierFactor` (novelty=True) `.decision_function` (<0 outlier, use as-is), **`IsolationForest`: use `score_samples` and negate**, or use its `decision_function` — pin the exact call per engine so the tail direction is never inverted.
