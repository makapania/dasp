...
Historical sections moved verbatim out of `PROJECT_STATUS.md` so the living status doc stays small enough for a session to read at start-up. Nothing here describes current state — for that, read `PROJECT_STATUS.md`. Grep this file for past hand-offs, per-PR details, completed work and superseded sections.

**Batch 1 — 2026-09-15.** Blocks below appear in their original order in `PROJECT_STATUS.md` at commit `30dad1a`. Where a moved block had no heading of its own, a `## [Moved block] …` heading was added to say where it came from.

---

## [Moved block] From “ACTIVE DIRECTION (2026-09-12) — Python 3.14 migration” — PR #65 review/merge-gate notes
...
>
> **What works (unchanged):** humans use the GUI (`python spectral_predict_gui_optimized.py`) — untouched by this work. Agents compose backend primitives directly and write their own orchestration; an active external research program does exactly this at scale (528-combination DD-SIMCA search, Bayesian PLS-DA, 32-page manuscript validated on 948 independent external objects).
>
> **What changed:**
> 1. **CLI retired.** `src/spectral_predict/cli.py`, `tests/test_cli_help.py`, the `pyproject.toml` console script, and all CLI docs (README + `docs/MACHINE_LEARNING_MODELS.md` "Example 4") are gone. It had **four** independent defects and could never complete a run — see SESSION_LOG 2026-07-29. Rationale (user's): a CLI can only encode a fixed analysis shape and there is no standard analysis. The headless-import guarantee that `cli.py`'s docstring carried (PR #56) is preserved as a test.
> 2. **`docs/AGENT_COMPOSITION.md`** — the primary deliverable. Declares the stable composition surface, documents owning your own CV (incl. grouped CV, which no entry point supports), and lists the traps. **Every example was executed against the repo's own `example/` data**; 5 of 10 first drafts failed, which is why the doc is empirical rather than written from inspection. Linked from `CLAUDE.md`.
> 3. **`search._multiclass_varsel_mask` → public `multiclass_varsel_mask`**, private name kept as a delegating alias so the external pipeline keeps working untouched. Selective `__all__` on `search.py` only. New `tests/test_agent_composition_api.py` (28 tests) pins the documented surface so a refactor here fails HERE instead of silently on another machine.
> 4. **`run_search` docstring corrected** — it promised a bare DataFrame but returns `(df_ranked, label_encoder)`.
>
...
>
> **Known loose end:** `interactive.py` / `interactive_gui.py` are now orphaned (imported only by the retired CLI). Left in place — deletion needs explicit permission — with docstrings marking them unimported and unsupported.
>
> **Cross-family review (both folded in):** Codex — "needs changes", 2 doc-accuracy blockers, both real: (1) the MACHINE_LEARNING_MODELS.md rewrite reintroduced `df = run_search(...)`, the exact tuple trap the new guide exists to prevent; (2) the guide claimed all variable selectors return `(n_features,)` importance arrays, but `ipls_forward`/`ipls_backward`/`mc_sipls`/`mwpls` take `wavelengths` as a required third positional and return a list of subset dicts. GLM 5.2 — "ship with fixes", found 1 blocker Codex missed: the guide promised `n_select` could be omitted while the signature made it required (following the doc raised `TypeError`) — signature now `n_select=None` to match. GLM also caught that `rank` is NOT always present on interval-subset dicts (absent on `ipls_forward` combined entries) — a conditional key that execution reported as always-present because the test run stopped early. **Lesson (SESSION_LOG): executing examples and reading source catch different classes of doc error; do both.** Reviewers disagreed on a `DeprecationWarning` for the `_multiclass_varsel_mask` alias — kept silent per Codex (would break warning-strict callers; the alias exists to not disturb a live off-repo pipeline).
>
...
>
> **GUI impact — checked, not assumed.** GUI tests were skipped (nothing here changes GUI code, and backgrounding them spawns Tk windows whose manual closure kills the run — one died at 36%, exit 127). But note the GUI **does** reach `search.py`, and at `gui:30550` it imports the *private* `_multiclass_varsel_mask` directly, calling it at `gui:30573`. A hard rename would have broken the GUI's decision-view path. The back-compat alias added for the off-repo pipeline covers the GUI too; verified the exact import still resolves, that all four symbols in that GUI import block (`_WOLD_METHODS`, `_multiclass_preprocess_matrix`, `_multiclass_varsel_mask`, `build_multiclass_decision_view`) still import, that the GUI's positional call site is compatible with the new `n_select=None` default, that the GUI module still compiles, and that the 49 multiclass/foldins backend tests behind that path pass. **Lesson: "we didn't touch the GUI" was too loose — the GUI imports private backend names, so check the call sites before trusting it.**
>
> ## ▶ ACTIVE DIRECTION (2026-07-07) — T-31 PR #64 **MERGED** to main (review fold-ins GPT-5.5 + Kimi complete; user did manual GUI pass)
> **PR #64 REVIEW + FIXES (2026-07-07, Fable orchestrating):** ran a PR review of #64 plus a Codex GPT-5.5 (medium) independent review, then folded in Kimi K2.7 cross-family review of the fixes. **User confirmed the live GUI visual pass looked fine** (the last outstanding item) → merge cleared. **GPT-5.5 found 2 real robustness bugs (both fixed):** (F1) malformed targets — an object-dtype target mixing types (e.g. `1` and `"2"`) or containing NaN/None hit `np.unique(y)` and aborted the whole multiclass run with an opaque `TypeError` before any row-guard ran; new `simca.check_multiclass_labels(y)` now raises a clear `ValueError`, wired into `MultiClassClassModel.fit`/`.cross_validate`/`.evaluate_novelty` + `run_multiclass_simca_search`. (F2) `run_multiclass_simca_search(..., varsel_paths=["importance"])` with the documented-optional `variable_selection_n_select=None` did `int(None)` in `_multiclass_varsel_mask`, skipped every row, and returned an EMPTY leaderboard; now defaults to `min(100, n_features)` (matches the GUI's Top-N default). GPT-5.5's 3rd finding (predict/decision-view compute the decision matrix twice — efficiency, not correctness) deferred as a separate perf ticket. **Kimi review of the fixes: both correct + complete, no CRITICAL/HIGH.** Folded in 2 MEDIUM completeness gaps: (M1) the F2 default only caught `None` not `NaN` — the top-decision-view rebuild reads `NSelect` back as float64 so an omitted Top-N is `NaN`; now `_multiclass_varsel_mask` also defaults on NaN and the top-view path coerces NaN→None. (M2) `compute_validation_metrics_for_top_models` is a second public entry point reaching `np.unique(y)` unguarded; added `check_multiclass_labels` there too. GUI path byte-identical (always passes real int Top-N + homogeneous labels; validation is additive). **6 new regression tests; targeted multiclass suites 116 passed** (`test_multiclass_search` + `test_simca` + `test_multiclass_foldins` + `test_multiclass_export`). One pre-existing unrelated GUI test failure (`test_tab9_rejects_multiclass_primary` — patches `zipfile.ZipFile` but `load_model` raises `FileNotFoundError` first; not touched by this diff). Detail: SESSION_LOG 2026-07-07. **PR #64 MERGED to main 2026-07-07 (merge commit `5a69bc9`, per-task history preserved). Deferred: GPT-5.5 F3 perf ticket (decision matrix computed twice on predict/decision-view paths — add a `labels_from_acceptance` helper); pre-existing `test_tab9_rejects_multiclass_primary` test-patching bug.**
>
> ## ▶ ACTIVE DIRECTION (2026-07-06) — T-31 PR OPENED (#64 retitled), review nits closed; awaiting user merge greenlight
> **PR + NITS (2026-07-06, Fable orchestrating, Opus impl/review):** two non-blocking peer-review nits landed in commit `2809ee8`: (1) `test_generated_script_reproduces_deriv_config` docstring said "reproduce identically" while asserting `rtol=1e-6` — wording now matches (assertion untouched); (2) the generated reproduction script's `variable_selection` list→bool-ndarray coercion is now guarded to all-bool lists (a hypothetical future int-index list can no longer be silently corrupted into a mask; invariant documented; two new tests exec the actual generated coercion block). 16 export+decision-view tests green. **PR #64** (previously the planning-docs PR from this branch) retitled/rebodied to "T-31: Multi-Class SIMCA UX parity + saved-model consumers" — https://github.com/makapania/dasp/pull/64. NOT merged, no auto-merge. **Still owed:** live GUI visual pass (functional pass done, screenshots blocked headless) — flagged in the PR body; user merge greenlight.
>
> ## ▶ ACTIVE DIRECTION (2026-07-06) — T-31 Run-Selected multiclass mask-varsel rows FIXED; awaiting user merge greenlight
> **RUN-SELECTED DECISION-VIEW BUG FIXED (2026-07-06, Fable; Opus review of primary fix):** user reported double-clicking a multi-class leaderboard row opened a decision view for Wold rows but NOT `importance` rows. **Confirmed broader — ALL discrimination (mask-based) varsel methods** (importance/cars/spa/uve/uve_*/ipls/fipls_*/ipls_forward/ipls_backward/mc_sipls/mwpls). Root cause: `build_multiclass_decision_view` stores the *resolved* `variable_selection` (a boolean ndarray) on `view['config']`; `_multiclass_decision_header` did `ndarray or 'none'` → `ValueError: ambiguous truth value`, swallowed by the render `except` which destroyed the half-built window → user saw nothing. `_mc_worker_running` is NOT stuck (reset before the show call). Fix (`spectral_predict_gui_optimized.py`): header renders an ndarray as `"mask (N vars)"`; run-selected worker stamps the readable method name onto `view['config']['varsel_path']`; header-builder prefers it. **Second defect (Opus review, newly reachable):** the now-openable window's Export Repro Script/Notebook embedded `repr(mask ndarray)` → bare `array([...])` (undefined name) → `NameError` when run; fixed in `code_generator.py` via `_reprsafe_multiclass_config` (`.tolist()`) + template list→bool-ndarray coercion, script + notebook both covered. Regression tests all red-first (crash-site header, importance/cars end-to-end, generated script+notebook mask reproduction bit-exact). **Targeted suites 134 passed.** Detail: `.superpowers/sdd/run-selected-varsel-investigation.md` + SESSION_LOG 2026-07-06. Do NOT auto-merge.
>
> ## ▶ ACTIVE DIRECTION (2026-07-07) — T-31 derivative-varsel bug FIXED; merge gate PASS; awaiting user merge greenlight
> **cp1252 PRINT FIX (follow-up, commit `e57b8f4`, Fable + Opus impl/review):** the derivative-varsel fix below made fiPLS/interval selectors actually run, exposing latent U+2192 arrows in 3 `print()` f-strings in `variable_selection.py` (lines 1028/1119/1219 — `uve_cars_spa`/`fipls_spa`/`fipls_cars` summaries) that raise `UnicodeEncodeError` on cp1252 stdout (Windows console default; recurring bug class — see `.sco` import work). Arrows -> ASCII `->`; display strings ONLY, selection logic/indices/importances untouched. Swept the whole varsel path: search.py/preprocess.py/tpe_preprocessing_discovery.py clean (R²/×/• ARE cp1252-safe, left alone); `wavelength_selection.py` ✓/✗ and `io.py` ⚠️ prints are off the run path, left alone. New `tests/test_cp1252_prints.py` (AST scan of print/log literals incl. f-string parts + runtime fiPLS calls under strict-cp1252 stdout) — confirmed RED pre-fix. Targeted suites 100 passed. Detail: `.superpowers/sdd/cp1252-print-fix-report.md`.
> **DERIVATIVE-VARSEL BUG FIXED (2026-07-07, Fable; fix `4c8ba73` + test `b9e0c40`):** the open review finding below is closed. After SG-derivative preprocessing `_multiclass_preprocess_matrix` trims the wavelength axis (e.g. 40→34), but four sites passed the FULL untrimmed `wavelengths_full` into `_multiclass_varsel_mask` against the trimmed `X_pp`; interval varsel methods (ipls_forward/ipls_backward/mc_sipls/mwpls/fipls_spa/fipls_cars) then indexed past the trimmed matrix → `IndexError` → caught as `MulticlassVarselUnsupported` → derivative+interval combos SILENTLY SKIPPED in the main search / broken rebuilds. Fixed by passing the trimmed axis returned by the SAME preprocess call at each site: `_multiclass_holdout_metrics` (`_wl_tr`), `run_multiclass_simca_search` main loop (`wavelengths_current`), top-decision-view build (`_top_wl`), GUI `_run_selected_multiclass_result` (`wl_trimmed`). These are the only 4 production `_multiclass_varsel_mask` call sites. Mask width was already trimmed-width; only the interval-index axis was wrong, so save/reload/predict + decision-view alignment unchanged. **Regression tests (stricter than no-crash — old failure was a silent skip):** new tests assert a leaderboard row IS produced for deriv+interval and mask width == trimmed count (34); confirmed FAIL pre-fix (`IndexError: index 34 is out of bounds for axis 1 with size 34`). Opus implementer + separate Opus reviewer (APPROVE, no blocking findings). Targeted suites **174 passed**. Full detail: `.superpowers/sdd/varsel-axis-fix-report.md` + SESSION_LOG 2026-07-07. **Still owed:** user merge greenlight (do NOT auto-merge).
...
> ## ▶ ACTIVE DIRECTION (2026-07-06) — T-31 Multi-Class SIMCA **UX-PARITY feature COMPLETE (Tasks 1-12 + final-review fix) on `feat/T31-multiclass-simca`** (committed, NOT merged, headless-verified). Made multi-class configure/sweep/report/validate/present like its siblings. Spec `docs/superpowers/specs/2026-07-06-multiclass-simca-parity-design.md` (A-J), plan `docs/superpowers/plans/2026-07-06-multiclass-simca-parity.md` (13 tasks). **What landed:** (A) config moved off Import page into 4A Model Config + 4B Variable Selection subtabs; (B/C) alpha + n_components swept via preset-checkbox+custom lists mirroring one-class `_collect_simca_overrides` (default 0.99 → default run cost unchanged); (D) variable-count reuses shared Top-N checkboxes; (E) **full varsel set** wired via `variable_selection.py`'s real selectors (importance/cars/spa/uve/uve_spa/uve_cars/uve_cars_spa/ipls/fipls_spa/fipls_cars/ipls_forward/ipls_backward/mc_sipls/mwpls + Wold×3) — **select-on-calibration per chemometrics convention, NOT per-fold nesting (decided w/ user; not ML leakage — external Validation tab is the honesty check; see `feedback_varsel_select_on_calibration.md`)**; dropped cars_tree/uve_cars_tree (no-op for PLS SIMCA), vcpa/ga (other modules); (F) `run_multiclass_simca_search` list-capable grid preproc×engines×varsel×sizes×alphas×ncomp, scalar callers byte-identical; (G) min-class-n spinbox in 4A; (H) new `NComponents`/`NSelect` columns + Alpha/NComponents/Engine/VarSelMethod surfaced in leaderboard + tooltips + decision-view config header; (I) `multiclass_simca` branch in `compute_validation_metrics_for_top_models` (val_* decision-matrix metrics on holdout; KS/Random/Stratified; **SPXY disabled** — d_y undefined for categorical; run-global baseline/smoothing/min_class_samples threaded through so holdout pipeline matches ranked row; known-class-not-novelty honesty note); (J) removed run-completion auto-popup (double-click a row like other methods). **Final whole-feature review (Fable) caught 1 cross-task blocker — the double-click Run Selected/Save path rejected the 13 newly-offered varsel methods — FIXED (`4d0115d`): guard widened to the accepted-paths union + row mask resolved via `_multiclass_varsel_mask`.** 261 tests green (multiclass/simca/model_io/contamination) + integrated e2e smoke (24-row swept grid + holdout val_* all populated). Per-task Opus implement+review; commits 4230fa6..4d0115d.
> **CONSUMERS DONE (2026-07-06, commit `cc0f486`):** the 3 saved-model consumers are verified/wired. (1) **Predict tab worked already** — backend `predict_with_uncertainty` multiclass branch + GUI `_display_uncertainty` `'p_values'` branch + string-label `_display_predictions` render the decision matrix; verified e2e via a real save→load→predict round-trip. (2) **Tab 9 scoped-and-EXCLUDED** — new `_comparison_reject_multiclass()` refuses a `multiclass_simca` model at primary/auxiliary load with an explicit "run it on the Predict tab" message (Tab 9 is scalar-prediction/R²-consensus machinery; a decision matrix has no home there). (3) **Notebook export worked already** — verified nbformat-v4 valid + the code cells EXECUTE (subprocess → `decision_matrix.csv`); hardened the test. Also fixed a pre-existing stale test (`test_multiclass_task_vars_exist` asserted scalar `mc_alpha`/`mc_n_components` that the parity refactor `1359d22` replaced with checkbox collectors). **194 tests passed (+3 new); 1 PRE-EXISTING dual-import test-ordering flake** (`test_run_selected_accepts_discrimination_varsel` — `super()` TypeError on `spa` when `test_multiclass_search.py` runs first; passes standalone + in a fresh production process → NOT a product bug; separate test-infra ticket to unify `src.spectral_predict` vs `spectral_predict` imports). Full detail: `.superpowers/sdd/consumers-report.md` + SESSION_LOG 2026-07-06. **Still owed:** live GUI `run`/`screenshot` pass; user merge greenlight (do NOT auto-merge).
> **FOLLOW-UPS DONE (2026-07-06, Fable, commits `ce96db5` + `afa02ea`):** (A) the "pre-existing dual-import test flake" is FIXED — and its real root cause was NOT the src/spectral_predict dual import: `test_multiclass_gui_parity.py`'s thread-recording monkeypatch of global `threading.Thread` collided with joblib's `backend="threading"` ThreadPool (`multiprocessing.dummy` calls unbound `threading.Thread.start(self)` on a `DummyProcess`, hitting `_RecordingThread.start`'s `super()` with a foreign self). Fixed in the test; ALSO unified the import namespace (9 GUI + 3 backend `src.spectral_predict` → unprefixed) as a latent-frozen-bundle-bug cleanup. Combined run `test_multiclass_search.py` + `test_multiclass_gui_parity.py` now GREEN both orders (54; 55 with new test). (B) cosmetic minors closed: `varsel: None`→`none`, `pack()` winfo_manager guard, discrimination honesty caption → Caption style, and the redundant none/Wold×Top-N duplicate rows collapsed (new collapse test). (C) Tab-9 multiclass side-by-side comparison SCOPED ONLY (exclusion stays) — verdict-grid data model + agreement metrics + new Tk sub-view, ≈Phase-D2 effort; see `.superpowers/sdd/outstanding-report.md`. 132 multiclass-suite tests green. **Still owed unchanged:** live GUI `run`/`screenshot`; user merge greenlight.
> **~~NEXT~~ (superseded by CONSUMERS DONE above): verify + wire the 3 saved-model consumers for multiclass.** (1) **Predict tab** (`_run_predictions` 42747 / `_run_prediction_workflow` 49248 / `_load_model_for_prediction` 42231): backend `model_io.predict_with_model` DOES handle `multiclass_simca` (returns `{p_values, decision_matrix, ...}`, model_io.py:767-780/915-921); verify the GUI Predict consumer displays the decision matrix for a saved `.dasp` (merge-gate added a partial predict-tab branch — confirm e2e). (2) **Multi-Model comparison (Tab 9, 50721)**: R²/ensemble-oriented (consensus by R², `ensemble.predict`) — almost certainly does NOT support decision-matrix models; either add a multiclass branch or cleanly exclude+message. (3) **Colab notebook export**: `generate_multiclass_reproduction_notebook`/`_script` exist + wired into decision-view Save/export (30302-30330) — verify it produces a valid runnable notebook for a saved multiclass model "and all". **Also owed:** live GUI `run`/`screenshot` pass (headless only this session); user greenlight to merge (do NOT auto-merge); deferred cosmetic minors (Task 3 display-test could be stdout-capture; Task 8 unconditional pack + caption styling; Task 12 `varsel: None`→`none`; redundant none/Wold×sizes duplicate rows).
> **POST-FEATURE FIX (2026-07-05, commit `294d65f`):** independent verification of the run-selected/Save-Model feature caught a real bug the delegated round-trip test missed (it only checked `predict_with_model(validate_wavelengths=True)`). With `validate_wavelengths=False` (the common path) the DataFrame branch of `predict_with_model` SKIPPED the full-spectrum edge-mask handshake — so a DERIVATIVE-preprocessed model (SG edge mask trims the axis, e.g. 40→34 cols) received the full-width matrix and its per-class StandardScaler raised "X has 40 features, expecting 34". Fixed: the `validate_wavelengths=False` DataFrame branch now honors `use_full_spectrum_preprocessing` (preprocess full → subset to trained wavelengths), mirroring the ndarray branch; only activates when the handshake metadata is present, so single-Y paths are unaffected. Save→load→predict now reproduces the in-app decision matrix on RAW spectra for raw/snv/deriv/snv_deriv × float/str columns × validate True/False × DataFrame/array. So the earlier "verified bit-exact" claim was only true for `validate_wavelengths=True` until this fix. 72 GUI+model_io+foldin tests green.
> **RUN-SELECTED-RESULT follow-up (2026-07-05, commits `e83f6f9` feature + `e8d2b46` Codex/Kimi fold-in):** double-clicking a `multiclass_simca` leaderboard row now rebuilds THAT row's exact config, shows its in-sample decision matrix (reusing `_show_multiclass_decision_view`), and offers a **Save Model (.dasp)** button that persists a `MultiClassClassModel` whose stored preprocessor + `use_full_spectrum_preprocessing`/`full_wavelengths` let `predict_with_model` classify RAW new specimens (preprocessing + SG edge mask reproduced on load — verified bit-exact). Intercepts at `_on_result_double_click` on the ROW's `Task` (single-Y Refine path byte-identical). Cross-family review (Codex 5.5 + Kimi K2.7): thread-safety + save/predict handshake sound; folded in HIGH radio-clause routing regression, HIGH numeric-string-wavelength coercion, MEDIUM atomic-stash guard, MEDIUM strict varsel validation, LOW busy-flag. 23 GUI + 120 adjacent tests green. Real-data e2e (ORAU Excel): select non-top row → save → load → predict reproduced the decision matrix exactly. **A live GUI `run`/`screenshot` pass is still owed.**
> **MERGE-GATE DONE (2026-07-05, commit `c2ade23`):** whole-diff panel Codex 5.5 + Kimi K2.7 + MiniMax M3 + pr-review-toolkit (silent-failure-hunter, pr-test-analyzer). No CRITICAL/HIGH survived; verified findings folded in — Wold-diagnostic-collapsed-to-1-PC on the 0.99 default (triple-converged: pr-test/Kimi/MiniMax), empty-null-silent-all-reject now marks the class unmodelable, predict-tab consumer branch for multiclass (MiniMax), export preserves sample index + label dtypes (Codex), plus UX silent-failure surfacing (no success chime on crash, wrapped export callbacks, Wold/no-view warnings, n_components 1.0/≤0 guards). Refuted 1 false positive (Kimi tier-change — already no-ops via caught ValueError). **Diff-failure-set vs origin/main: HEAD ex-GUI 5 failed / 2768 passed; the 5 are the pre-existing T-CI-3/T-CI-4 set, confirmed identical on origin/main → ZERO new failures.** Known-deferred (separate tickets): reproduction-script size for large real data (CSV-sidecar option), Refine-tab predict consumer multiclass branch.
> **Phase D DONE (2026-07-05, commits `2819090`/`1c7c326`/`ff8875c`/`5af8ec8`):** D1 5th task radio + multiclass control panel (α / n_components 0.99 default / min-class-n / varsel-path picker / per-class engine picker); D2 decision-matrix + Wold MPOW/DPOW results view (`build_multiclass_decision_view` provider + `df.attrs["top_decision_view"]`, worker-thread dispatch, auto CSV export); D3 self-contained reproduction script/notebook (`generate_multiclass_reproduction_script`/`_notebook`, decision matrix bit-identical via the same backend, export buttons in the decision-view window). Deferred fold-ins closed: `predict_with_uncertainty` multiclass branch, `_cross_fit_null` EE-fold-failure warning (PCA-reduce rejected — would desync null from fit), LOCO `oof_cv=` reuse (halves per-config OOF CV). Audited & no-op: `model_config`/`cv_utils`/`templates/validation` are not reached by the multiclass path (verified, do NOT thread them). Single-Y (regression/classification/one-class) paths byte-identical. 19 new tests + zero regression across simca/model_io/contamination. **GUI verified headless only this session (user asleep) — a live `run`/`screenshot` launch is still owed at the gate.** Real-data e2e: 757×2151 ORAU → 10-class decision matrix; held-out Arcy 62% novel at ~5% in-sample false-novel.
> **True multi-class class-modeling** (Oliveri & Downey 2012 decision framing / Pomerantsev & Rodionova DD-SIMCA), the "none of the above" capability discriminant classifiers (PLS-DA/RF) can't express. Spec `docs/superpowers/specs/2026-07-04-T31-multiclass-class-modeling.md` (review-hardened: Fable/Codex-5.5/DeepSeek on the plan, Codex-5.5/MiniMax on the spec). Plan `docs/plans/2026-07-04-T31-multiclass-simca-implementation.md` (Phases A→D). Execution: Opus orchestrates, GLM-5.2 write-mode workers implement, Opus reviews each task + multi-family panel at phase gates.
> **Phase A (backend core) done, tasks A1–A8** — new `src/spectral_predict/simca.py` `MultiClassClassModel`: per-class DD-SIMCA membership (`PCASIMCA.p_joint` added in A1, `decision_function` byte-identical) + pluggable per-class engines (OCSVM/IF/LOF/EE, each calibrated to a per-class empirical p-value), scaling modes (none/per_class/global, fit train-only), `n_components="per_class_cv"` one-vs-rest tuning, nested leakage-safe `cross_validate`, `evaluate_novelty` (LOCO/external), dedicated metrics (`multiclass_simca_metrics`/`wilson_ci`/`novelty_tradeoff_auc`), `.dasp` persistence + forward-compat gate (`model_io.py` A8). Commits `426af15`(A1)…`4f96d45`(A8), hardening `cbb69bf`. **34 `test_simca.py` + 109 contamination/model_io = 143 green, zero regression.**
> **Phase-A multi-family gate (Codex 5.5 + DeepSeek V4 Pro + Kimi):** cleared the core (byte-identical `p_joint`, correct engine signs, leakage-safe nested CV, legacy-safe gate); found + fixed small-n/NaN edge bugs the green tests masked — DeepSeek's key catch: non-SIMCA empirical-p floor `1/(m+1)` can't reach α=0.05 below m=20. **User-approved `min_class_samples` policy = LAYERED:** non-SIMCA n<20 → unmodelable+warn; SIMCA warns at n<max(20,5·n_comp) but still models; per-class calibration surfaced via A7 metrics + Wilson CIs. Also fixed: per-fold null scaling (leakage), NaN-collapse in AUC/efficiency, `engine_params` forwarding, singleton-class CV guard.
> **Real-data validation** (user's `Contaminated Samples Raw_ORAU Added.xlsx`, `Site` classes): SIMCA flags **53–86% of held-out-site samples as novel** ("none of the above") vs a discriminant baseline forcing 100% into a trained site; in-site false-novel ~9% (≈α). Raw FTIR; preprocessing + tuned search (Phase C) will sharpen it.
> **Phase B (variable selection) done, tasks B1–B3** — all additions in `simca.py`, commits `72e6c1c`(B1) / `265d687`(B2) / `93d43ea`(B3), gate fold-in `eb72c05`. B1 `wold_modeling_power`/`wold_variable_powers`/`wold_variable_selection` (classical Wold 1976 MPOW + one-vs-rest cross-fit DPOW using RMS-about-zero; modes modeling/discriminating/balanced). B2 `wold_diagnostic_plot_data` (K×n_features arrays for the Phase-D GUI). B3 `variable_selection` prefilter on `MultiClassClassModel` — Wold modes + supervised `importance` on the genuine multi-class label + a **precomputed boolean-mask hook**; mask computed train-only, recomputed per fold (leakage-safe), gated by an empirically-calibrated novelty guard (supervised does NOT degrade novelty vs full-spectra baseline). `variable_selection=None` byte-identical to Phase-A. **55 `test_simca.py` + 145 adjacent green, zero regression.**
> **Phase-B multi-family gate (Codex 5.5 + Kimi K2.7 + MiniMax M3, rotated off GLM):** no BLOCKER; core Wold math + leakage sound. Fixed with red-first tests: below-floor-class varsel crash (`KFold(1)`), empty/invalid `n_select`, non-finite importances, Wold PCA `random_state`. Pushed back (verified) on two MiniMax HIGHs — DPOW RMS-vs-std (probe refutes the std claim: std→ρ≈0, RMS→ρ≈0.93; kept RMS) and own/cross asymmetry (spec-pinned cross-fit; kept). **Methodology decisions RESOLVED with user (2026-07-04, commit `748289c`):** (a) keep DPOW RMS-about-zero; (b) `balanced` now min-max normalizes MPOW·DPOW before multiplying; (c) MPOW `ddof` small-n bias documented, dof-refit deferred; (d) supervised-varsel adversarial-novelty limitation documented (representative-case guard passes); (e) model-layer supervised ships `importance` only + a **precomputed boolean-mask hook** as the extensibility path for C.
> **Phase C IN PROGRESS.** **C1 done** (commit `69a4750`): `multiclass_simca` threaded through `scoring.create_results_dataframe` (dedicated NoveltyAUC/Efficiency/NoveltyRate/… schema + `engine_family`/`varsel_path` tags) and `compute_composite_score` (ranks by α-sweep NoveltyAUC, tie-break min-class-n, gap-penalty no-op) + `model_registry.MULTICLASS_ENGINES` / `get_supported_models`. Fall-through audit: multiclass schema has no `Accuracycv`, so scoring succeeding on it proves an explicit branch. 5 tests in new `tests/test_multiclass_search.py`, zero regression. Remaining C1 sites (`model_config`/`cv_utils`/`code_generator`/`templates`) are Phase-D/export path — threaded there.
> **C2/C3 done** (commit `04e4e04`): `run_multiclass_simca_search` (+506/−0, existing paths byte-identical) — grid = preprocessing × engines × varsel_paths (NO G^K; per-class n_components auto-tuned inside each row), OOF metrics + LOCO `NoveltyAUC` ranking; 66 tests green. **Real-data e2e (user's Excel, `Site`): functional gate PASSED** — search runs, NoveltyAUC ranks pca-simca>ocsvm, save→load reproduces the decision matrix exactly, LDA baseline forces 100% of held-out Arcy into a trained site.
> **Phase-C 3-family gate (Codex 5.5 + Kimi K2.7 + MiniMax M3, rotated off GLM) — NO BLOCKER.** Fold-in queue (unambiguous bugs, ready to implement w/ tests): LOCO AUC endpoint collapse (perfect-separation→0.0), NaN/additive-tiebreak ranking (→ lexicographic), all-NaN-leaderboard guard, vacuously-novel inflation (all-NaN foreign p → -inf), off-schema `unmodelable_classes`/`reason` cols, malformed-preprocess-config abort, `PCASIMCA` unseeded PCA, docstring/threshold/verbose polish + adversarial tests.
> **⚠ HEADLINE real-data finding + OPEN user decisions (interlock — NOT changed unilaterally):** **`n_components="per_class_cv"` cripples novelty** — held-out Arcy site: per_class_cv→17% novel vs fixed n_components=10→100% (both ~6% in-sample false-novel); root cause = one-vs-rest balanced-accuracy tuning optimizes within-known discrimination, not novelty. Decisions awaiting user: (A) document LOCO-is-an-optimistic-proxy for the §1 held-out-4th-class target; (B) class-balanced vs sample-weighted novelty_rate (recommend balanced for imbalanced Site data); (C) composite single-objective NoveltyAUC vs `NoveltyAUC·Efficiency^0.5`; (D) **per_class_cv novelty tuning — recommend option 1 (novelty-oriented n_components), contradicts locked A5 so needs explicit OK.**
> **Phase-C fold-in DONE (commit `4bf39db`), Phase C COMPLETE.** All gate bugs fixed (LOCO endpoint, lexicographic ranking, all-NaN guard, vacuous-novel, off-schema cols, preprocess-abort, PCASIMCA seed) + user decisions: (B) class-balanced novelty_rate; (D) **novelty-oriented `n_components` — float in (0,1) = per-class variance fraction, search default 0.99** → real-data held-out Arcy novelty **17%→100%** (calibrated ~7%); (A) LOCO documented as optimistic proxy; (C) unchanged (NoveltyAUC-primary; `·Efficiency^0.5` deferred). 180 tests green, zero regression, existing paths byte-identical.
> **NEXT: Phase D** — GUI 5th radio (`multiclass_simca`) + engine/α/n_components(0.99 default)/varsel controls; results view = decision matrix + single/multiple/novel + Wold MPOW/DPOW plots; code export. Fold in the deferred items during D: `predict_with_uncertainty` multiclass branch, EE PCA-wrapper in `_cross_fit_null`, remaining task-type sites (`model_config`/`cv_utils`/`code_generator`/`templates`), LOCO double-CV perf optimization. **Then merge-gate** (whole-diff multi-family + full pr-review-toolkit + local diff-failure-set vs `origin/main`; user greenlight only — do NOT auto-merge). Full detail: SESSION_LOG 2026-07-04/05 (Phase C entry).
>
> ---
>
> **Last updated:** 2026-07-01 — **Post-merge review fold-in on `feat/legacy-asd-sco-import`: 4 findings from a high-effort code review of the `.sco` import work, fixed + Codex-5.5-cleared.** (1) `read_asd_dir` wrapped the per-file read in a broad `except Exception` that swallowed the `ValueError` `read_legacy_asd` deliberately raises for corrupt/truncated legacy files — silently dropping them. Fix: capture `binary = _is_binary_asd(asd_file)` before the `try`, then `if binary: raise` in the handler so binary/legacy corruption surfaces loudly (per the reader's "raise, don't silently fall back" contract); ASCII read failures stay tolerant so one junk text file can't abort a whole folder. (2) **Calibration Transfer tab (4 detection sites) plus other GUI tabs + the export-naming registry still globbed only `*.asd`/`*.sig`**, so legacy `.sco` folders were undetectable even though the backend now supports them — the original `.sco` PR fixed 6 GUI globs but missed these. (3) `read_binary_asd`'s `NotImplementedError` message had dropped the `not yet implemented` substring that `tests/test_optional_r_bridge.py::test_native_reader_not_implemented` asserts via regex — the test is `skipif(not R)` so it's silently skipped on this Windows box but goes red on any CI runner / machine with Rscript; reworded to restore it. (5) **Centralized ASD extension handling:** new `io.ASD_EXTENSIONS = (".asd",".sig",".sco")` + `io.list_asd_files(directory)` (case-insensitive, sorted, `Path.iterdir`) replace ~13 hand-maintained extension globs across the GUI and `io.py`, so the next extension addition can't miss a site (this class of bug had already recurred — Kimi caught one in the original PR). **Verified:** 19 ASD tests pass; corruption re-raise + the R-bridge regex match confirmed live; Codex 5.5 static+import review returned **no findings** and confirmed no Path-vs-string breakage at any converted call site (each uses the result for truthiness/`len`/iteration or `str()`-wraps it) and no circular import. Diff +58/−32 across `io.py`, `readers/asd_native.py`, `spectral_predict_gui_optimized.py`. **Follow-up same day:** a medium-effort PR review of #61 flagged that finding (1)'s `if binary: raise` was a footgun — one corrupt `.sco` aborted the whole folder import (loaded 0), amplified by `_file_equalize_batch` dropping a whole instrument. Per user direction, reversed to a **skip-and-report** policy: `read_asd_dir` now skips an unreadable/corrupt file (binary or ASCII), prints a `[!] Skipped N ...` summary, and hard-raises only when *zero* files load (with the specific reason, not a generic message). Summary uses ASCII (`[!]`) not `⚠️` — an emoji `print` on cp1252 stdout raises `UnicodeEncodeError` and would abort the very corrupt-file path being hardened. **Second review round (Codex 5.5):** folded in 4 more — (1) `_normalize_filename_for_matching` was missing `.sco` (reference IDs like `sample.000.sco` failed alignment); now derived from `ASD_EXTENSIONS`. (2) a `None` return from the binary handler (SpecDAL fail / sub-484-byte fall-through) was silently dropped and omitted from the skip summary → now reported. (3) `read_legacy_asd` rejects entirely-non-finite payloads (`ValueError`) so all-NaN `.sco` files are skipped+reported instead of entering the matrix (partial NaN still tolerated). (4) converter lists per-file reasons on total failure. 23 ASD tests + 136 align/match tests pass. SESSION_LOG has the full writeup.
>
> ---
>
> ## Previous — 2026-06-19 — **Fixed: radio-button data-type toggle silently log-transformed plots (stale `use_absorbance`).** Importing a reflectance file (CSV/XLS/any format) could show absorbance-shaped plots while the radio read "Reflectance"; toggling the radio appeared to *convert* the data. Root cause was the hidden legacy `use_absorbance` flag (set by "Convert to Absorbance", never reset on a new file load) driving `_apply_transformation()` in the plot path — NOT the importer (stores values verbatim) and NOT detection (only labels). Fix: reset `use_absorbance` to False in `_apply_data_type_metadata` (all 11 load branches) + made the `_update_data_type_status_ui` reflectance branch reset it too (was asymmetric — only the `else` reset it). Verified against user's real file `1-s2.0-S0305440325002973-mmc2.xlsx` (140×2151 NIR, reflectance 0–1.19, correctly detected `reflectance` 66.7%). Legacy checkbox is hidden (never packed), so no workflow change. SESSION_LOG has full root-cause writeup.
>
> ---
>
> ## Previous — 2026-06-19 — **Legacy float32 ASD-v1 (`.sco`) import support added (uncommitted, on `main`).** User's `.sco`/numbered `.000` FieldSpec files read as all-NaN because they are the oldest ASD binary format (magic `ASD\x00`, float32 spectrum at offset 484) and SpecDAL assumes modern float64. Added native decoder `src/spectral_predict/readers/asd_native.py::read_legacy_asd()` (tried before SpecDAL in `_handle_binary_asd`; exact-size discriminator: `484+ch*4`→float32 decode, `484+ch*8`→None/SpecDAL, else→ValueError), `.sco` added to glob/`format_map`/`detect_format` magic/`_detect_directory_format`, one-off converter `scripts/convert_old_asd.py`. Standardized on `.sco`-only (unique stems; bare `.000` all share stem `italy` and would collapse to 1 row). Verified on real folder: 30×2151, 0 NaN, reflectance 0.06–0.97. **Also added `.sco` to all six GUI ASD-detection globs** (`spectral_predict_gui_optimized.py` ~16072/17663/41276/44994/45612/45685) — the GUI gates on its own glob before `read_asd_dir`, so the backend fix alone left `.sco` folders showing "No supported spectral files found" (caught by Kimi cross-family review). Tests: `tests/test_asd_legacy_binary.py` (9, incl. end-to-end `read_asd_dir` + NaN payload) + existing asd/io suites pass (80 passed, 3 skipped). Cross-family review (GLM + Kimi via opencode, codex auth was down): GLM's 2 CRITICALs were arithmetically self-refuting (float32/float64 sizes only collide at ch=0, excluded; float64 checked first); real findings = GUI gate (Kimi) + converter `--variant raw` stem collision (GLM+Kimi convergent, fixed). SESSION_LOG has full root-cause writeup.
>
> ---
>
> ## Previous — 2026-05-09 — **T-16 Phase 1 (CV-ANOVA p-value column) MERGED via PR #60 squash at `b24579a`.** Six-commit chain on the feature branch: plan v2 (Codex pre-review folded in) → initial implementation → docs → fix-of-fixes (5-reviewer post-implementation pass: code-reviewer + pr-test-analyzer + silent-failure-hunter + comment-analyzer + DeepSeek V4 Pro found 13 substantive items across 3 BLOCKER-equivalents) → revert NSGA-II + downgrade warning (Codex final-judgement pass caught my LV-count bug from the previous fix) → repeated-CV integration test pin. **What landed:** new `cv_anova_pvalue` column populated for PLS regression rows in BOTH grid path (`search.py:_run_single_config`) and Bayesian path (`unified_bayesian.py` objective + `convert_study_to_dataframe`, via trial user_attr). Per Eriksson, Trygg & Wold 2008. Pure additive — no change to which rows get computed, which row wins, or any search-loop behavior. Numerical impl uses `scipy.stats.f.sf` for tail stability. **PLS-DA explicitly excluded** because dasp's PLS-DA is a `PLSTransformer + LogisticRegression` hybrid (`models.py:1354`), not canonical PLS-on-dummy-Y. **NSGA-II rows excluded** because their RMSEcv is mean-per-fold (not pooled), so PRESS = N×RMSEcv² doesn't hold; harmonizing NSGA-II's CV metric is a separate ticket. Tests: 13/13 pass (`tests/test_cv_anova.py`: 9 unit + 4 integration: grid PLS, Bayesian PLS, grid negative-space PLS+Ridge, repeated-CV `cv_n_repeats=3`). Adjacent regression: 84/84 pass (`test_bayesian_dedup`, `test_t44_autoscale_wiring`, `test_cv_pls_clamp`, `test_nsga2_search`). Real-data smoke on BoneCollagen: PLS p-values [3.79e-24, 3.51e-04] median 3e-16 (extremely significant as expected); Ridge rows all nan. **Review depth:** 7 distinct review passes across 4 reviewer families (Codex × 2, pr-review-toolkit × 4 specialists, DeepSeek V4 Pro × 1) — caught the original BLOCKER (Bayesian-path bypass), the recurrence in NSGA-II (LV-count + RMSEcv-semantics mismatch), and a silent-nan-hides-upstream-bug pattern. **Phase 2 deferred** per user direction 2026-05-08: Path B (full-pipeline permutation refitting varsel per shuffle) would cost 25min-5hr per row for typical CARS/GA-PLS rows; user's typical regime (high R²cv on small N) means Q1 p-values mostly say `p << 0.001` and don't change decisions. Phase 2 plan + Codex review parked at `feat/T16-phase2-permutation` for future reference if it ever revives. **T-16 closed at Phase 1.**
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **T-16 commercial-software survey complete at `docs/T16_MODEL_COMPARISON_SURVEY.md`.** Headline finding: the user's premise that competitors expose pairwise model-comparison machinery dasp could copy is **mostly wrong**. Surveyed Unscrambler / SIMCA / PLS_Toolbox / OPUS / mdatools — all answer Q1 (model-vs-null: "is my model real or overfit") via permutation tests on Y, CV-ANOVA, F-tests on PRESS, or within-model component-selection tests. **None expose Q2 (model-A-vs-model-B: "is preprocessing X + model Y better than X' + Y'")** — the question dasp users actually ask when picking from the leaderboard. Q2 machinery lives in the ML/stats canon (Dietterich 1998, paired bootstrap, McNemar, Wilcoxon signed-rank). **Recommended smallest defensible shape:** Tier 1 paired bootstrap CI on ΔRMSEP / ΔAccuracy / ΔF1 (~1-2d, operates on existing held-out predictions from `compute_validation_metrics_for_top_models()` — no refits needed); Tier 2 paired permutation test on signed differences (~half day); Tier 3 McNemar's test for classification (~couple hours). NOT recommended: 5×2cv (Dietterich) requires 10× refits incompatible with dasp's existing run-once-then-rank workflow; Diebold-Mariano (author warned against using it for model selection); CV-ANOVA / `randtest` (answer Q1, not Q2). Total Tier 1-3 effort: ~3-4 days post-survey, on the original handoff estimate. **Three open user decisions** in survey doc §"Open decisions": Tier 3 scope, CSV export shape (sidecar vs leaderboard columns), GUI placement (inline Results tab vs dedicated Compare sub-tab). Survey commit: pending below this update.
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **T-TPE-VERIFY closed: `a33d956` empirically validated, neutral-to-positive across three splits + tree-arm.** Verification harness `tools/_repro_tpe_fix_downstream_ab.py` run on real BoneCollagen data with PRE-FIX (mean-prediction proxy) vs POST-FIX (model-family-aware) arms across SPXY / stratified / random 20% partitions; tree-arm verification via separate `tools/_repro_tpe_fix_tree_arm.py`. Strict gap≤0.02 numbers from `tools/_analyze_tpe_verify.py`: **SPXY** PRE=0.9722, POST=0.9699 (Δ=−0.0023, FAILS literal 0.97 threshold by 0.0001 — inside the ±0.005 chemometrics noise band per `feedback_neutral_means_user_facing.md`); **Stratified** PRE=0.9520, POST=0.9592 (Δ=+0.0072, PASS); **Random** PRE=0.9526, POST=0.9594 (Δ=+0.0067, PASS). POST has more gap-passing rows on every split (SPXY 73→79, stratified 149→162, random 121→190). Wall-clock POST is 0.80–0.87× of PRE on stratified/random (PLS proxy via SVD beats LGBM proxy on chemometrics-sized data; well under 1.5× ceiling). **Tree-arm:** `resolve_tpe_proxy_family(['LightGBM'])='tree'` confirmed; TPE Proxy family reported as `tree`; top-10 TPE RMSE spans 2.1381–2.8258 (non-degenerate, no mean-prediction collapse). **"Canonical winner missing" finding is benign:** POST picks `snv_deriv2_w17+autoscale` instead of literal `snv_deriv2_w15+autoscale` — same SNV+S-G+autoscale family, 2-channel S-G window difference; both arms' main grid evaluates all preprocessing. **Decision: close T-TPE-VERIFY without further tuning.** Resolver default change would contradict user design intent ("the whole point was to change behavior by being model specific"). Per-trial fallback heuristic is overengineering for a 0.0001 noise-band miss with clear wins on two of three splits. Artifacts (untracked per `_`-prefix convention): `tools/_tpe_fix_ab_arm_{PRE,POST}_{spxy,stratified,random}.csv`, `tools/_analyze_tpe_verify.py`, `tools/_repro_tpe_fix_tree_arm.py`, `tools/_tpe_fix_ab_*_run.log`. Full SESSION_LOG entry at top.
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **TPE proxy: model-family-aware fix shipped at `a33d956` (`feat(tpe): make TPE preprocessing-discovery proxy model-family-aware`).** Single behavior-change commit collapses what the original plan had as commits 1+2 into one cohesive change per user direction ("the whole point was to change behavior by being model specific"). What landed: `resolve_tpe_proxy_family(models_to_test)` taxonomy resolver (TREE = LightGBM/XGBoost/CatBoost/RandomForest; LINEAR = PLS/PLS-DA/Ridge/Lasso/ElasticNet; mixed/unknown/SVM/MLP/NeuralBoosted → linear); `_quick_evaluate(*, proxy_family='linear')` keyword-only branching; tree path restored adaptive `min_child_samples = max(2, n_train_per_fold // 5)` (the formerly-reverted 9b9d244 formula, now correctly aligned because tree branch only fires when downstream is tree-family); one_class path family-independent (LGBM-supervised + IF fallback regardless); `evaluate_config_with_seed` threaded through; both `search.py` call sites at `1920/1937` (regression/classification) and `5867/5884` (one-class) compute `proxy_family = resolve_tpe_proxy_family(models_to_test)` and pass through; audit-trail columns `tpe_proxy_family` and `tpe_proxy_model_name` explicitly copied at the four `preprocess_configs` builder sites and four result-row dict sites in `search.py`; proxy-collapse banner is now family-aware. **Tests:** 82/82 pass — 61 baseline + 21 new across `TestProxyFamilyResolver` (10), `TestQuickEvaluateFamilyRouting` (7 incl. the load-bearing n=49 distinct-scores Symptom 1 pin), `TestEvaluateConfigWithSeedFamilyRouting` (4). **End-to-end smoke** through `run_search(tpe_preprocess=True, models_to_test=['PLS'])` confirms `tpe_proxy_family='linear'` lands in result CSV. **Out of scope per user direction:** Basic Preprocessing Discovery (`smart_preprocess=True` / `preprocessing_discovery.py:669`) has the same bug but is intentionally NOT fixed here. Exhaustive Preprocessing (`ga_preprocessing.py:289` `evaluate_fitness`) already exposes `fitness_model='pls'` default + `_evaluate_with_actual_model` opt-in path — prior art that corroborates Q1 linear-default and a fallback path for any user hitting the Basic-path bug. **Cross-family review:** Codex CLI + Kimi K2.6 via opencode-call reviewed the plan with two convergent NEEDS_CHANGES findings (BLOCKER A/B harness monkeypatch + MEDIUM CSV manual-per-key plumbing) — both folded in. DeepSeek V4 Pro architectural review of the interim bit-identical refactor confirmed the design composes additively. Verification harness `tools/_repro_tpe_fix_downstream_ab.py:59` had its monkeypatch updated to accept `**kwargs` (preserved in untracked working tree per `_`-prefix-tool convention). Open follow-ups: SPXY 20% A/B verification on real BoneCollagen data (R²pred ≥ 0.97 at gap≤0.02 threshold), wall-time benchmark, GUI exposure decision (currently silent auto-routing per Q2). Plan history at `docs/plans/2026-05-08-tpe-proxy-model-family-aware-IMPLEMENTATION.md` (still references 5-6 commit structure; superseded by single-commit shape but kept as design record).
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **TPE proxy: implementation plan drafted, cross-family-reviewed, revised; ready to start coding.** Plan at `docs/plans/2026-05-08-tpe-proxy-model-family-aware-IMPLEMENTATION.md` answers all 7 design questions from the handoff with explicit rationale, threads `proxy_family` keyword-only through `_quick_evaluate` / `evaluate_config_with_seed` / `run_tpe_preprocessing_discovery` / `run_tpe_multistart_preprocessing_discovery` and the two TPE call sites in `search.py:1920/1937` (regression/classification) and `5857/5874` (one-class), and adds family resolver `resolve_tpe_proxy_family(models_to_test)`. **Adversarial review** by Codex CLI (cross-file dispatcher angle) + Kimi K2.6 via opencode-call (sister-site sweep angle) returned NEEDS_CHANGES with two convergent findings: (1) BLOCKER `tools/_repro_tpe_fix_downstream_ab.py:59` monkeypatch will TypeError on the new keyword-only `proxy_family` arg → folded into new commit 1.5 (patched function accepts `**kwargs`); (2) MEDIUM `_tpe_proxy_family` audit column does NOT auto-flow through CSV (existing `_tpe_*` extraction is manual per-key, not pattern-based) → §2.3 rewritten with explicit `tpe_proxy_family` copies at `search.py:1984/1998/5130/5164` (regression/classification) and `5904/5917/6205/6262/6769/6848` (one-class). One Kimi-unique sister-site finding: `preprocessing_discovery.py:669` (Basic Preprocessing Discovery / `smart_preprocess=True` in GUI) has the SAME proxy/downstream-mismatch bug. **User-locked decision 2026-05-08:** Basic explicitly OUT OF SCOPE for this PR; TPE-only fix. Rationale: Exhaustive (`ga_preprocessing.py:289` `evaluate_fitness`) already exposes `fitness_model='pls'` default + `_evaluate_with_actual_model` opt-in path, providing prior art that corroborates Q1 linear-default and a fallback path for users hitting the Basic-path bug. One Codex-unique finding: `test_quick_evaluate_unchanged_at_default` (`test_tpe_multistart.py:184`) and `TestQuickEvaluateDirect` tests have semantic-shift risk on default flip → §3.5 rewritten with explicit rename/repin instructions. Plan now at 5-6 atomic commits, ~410 LOC source + ~260 LOC tests. Q6 (`unified_bayesian.py` not affected) verified by both reviewers via `unified_bayesian.py:1529` `build_model(model_name, ...)` direct fit (no proxy). Verification battery preserved: BoneCollagen + PLS at SPXY 20%, R²pred ≥ 0.97 at gap≤0.02 threshold; tools/_repro_tpe_fix_downstream_ab.py harness updated in commit 1.5. Ready to start commit 1 (pure refactor).
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **TPE proxy: revert + cosmetic banner shipped; model-family-aware proxy designed and handed off (`docs/plans/2026-05-08-tpe-proxy-model-family-aware.md`).** Three pieces of work today on the same root cause: (1) `9b9d244` "fix" of LightGBM mean-prediction collapse via adaptive `min_child_samples` was reverted at `b879b52` after empirical regression on user's actual SPXY 20% partition (gap≤0.02: PRE best R²pred=0.9722 vs POST 0.9405, delta −0.032; root cause is proxy-vs-downstream-model mismatch — LightGBM votes for `snv_deriv3+autoscale`, PLS prefers `snv_deriv2_w15+autoscale`). (2) Display lie banner shipped at `258fc00` — when proxy collapses (`np.std(trial_values) < 1e-9`), GUI now shows honest "configs selected by random+diverse sampling" message instead of misleading per-config RMSE numbers. (3) Model-family-aware proxy proposal designed: tree-family downstream → tree-family proxy (the previously-reverted `9b9d244` change, now correctly aligned); linear/PLS family → PLS proxy; mixed defaults to PLS. Detailed handoff at `docs/plans/2026-05-08-tpe-proxy-model-family-aware.md` — user will have a new agent draft the implementation plan, then have Codex + Kimi K2.6 evaluate before any code is written. Cross-split A/B evidence preserved at `tools/_tpe_fix_ab_arm_*_{spxy,stratified,random}.csv` for the next agent to verify against. Methodology lesson captured in SESSION_LOG.md: a proxy that's smart but wrong about which features matter can be worse than a proxy that's broken-but-symmetric.
>
> ---
>
> ## Previous session — 2026-05-08 (latest) — **Exhaustive Preprocessing R²pred bug fixed — `chromosome_to_transform` closure was refitting StandardScaler on val.** User reported R²pred ≥0.11 lower for Exhaustive search than for TPE/Bayesian on the same data, while CV scores looked fine. Root cause: when the autoscale gene was set, the GA closure ended with `StandardScaler().fit_transform(X)`. `compute_validation_metrics_for_top_models` (`search.py:866-893`) called the closure twice — once on X_train, once on X_val — so each got its own freshly-fitted scaler. Val features ended up centered to **val's** column means/stds rather than train's; model trained on train-stat-scaled features predicted on val-stat-scaled features → R²pred collapsed. CV looked fine because search-time applied the closure once on full X_np before CV splits, so all folds shared the same (mildly-leaky-but-symmetric) scaler stats. **Fix (commits `3a4e502` + `ca987b4`)**: closure stripped of autoscale → per-spectrum-only; raw chromosomes return `None` regardless of autoscale gene; `evaluate_fitness` and search-time GA path apply autoscale via one-shot full-X `StandardScaler().fit_transform()` after the closure (preserves pre-fix CV behaviour, no fitness ranking shift); validation rebuild path uses `StandardScaler().fit(X_train_pre)` then `.transform(...)` on both train and val (train-fitted scaler reused on val). Old CSVs with autoscale=True chromosomes but missing `Autoscale` column: `search.py:806-822` backfills autoscale from `_decode_autoscale_gene(ga_genes)`. **No methodology change** — full-X autoscale before CV is preserved per chemometrics convention; only the train/val asymmetry was a real bug. **Cross-family verification**: Codex (NEEDS_CHANGES → addressed by `ca987b4` direct regression test → SAFE_TO_MERGE), DeepSeek V4 Pro Max max thinking via llm-call diff bundle (SAFE_TO_MERGE; cache safe, zero-variance edge case non-regression, `evaluate_fitness` bit-identical to pre-fix). 42 tests pass including new `TestAutoscaleTrainValAsymmetry` class with direct regression test against `compute_validation_metrics_for_top_models` (asserts R²pred matches sklearn `Pipeline([SNV, SavgolDerivative, StandardScaler])` reference within 1e-3). User-visible: R²pred numbers for `autoscale=True` rows change to honest values; CV/RMSEcv unchanged.
>
> ---
>
> ## Previous session — 2026-05-08 — **GUI runtime labels dropped + Phase 2 multi-seed A/B replicated on a second dataset (2025 Model Samples, n=139).** User flagged that the GUI runtime estimates (`~3-4 minutes` for smart preproc, `~1-3 min for 75 trials` for TPE preproc, `typically 5-30 seconds` for exhaustive) are misleading because (1) they prime user expectations the actual config doesn't match and (2) the exhaustive pre-pass has no per-cell progress feedback so users can't tell if it's hung vs progressing. All three labels replaced with non-quantitative warnings. User also pushed back on the framing of Phase 2 multi-seed rescore as "neutral" while still costing ~5-6x compute, prompting a new memory rule (`feedback_neutral_means_user_facing.md`): "neutral" requires parity on BOTH quality metric AND wall-clock; tied-on-quality-but-slower is a regression. **Empirical replication on 2025 Model Samples** (`tools/preprocessing_refactor_ab_2025models.py`, n=139 ASD spectra, Collagen Yield regression, n_train=97, n_external=42, gap-filtered passing set, score_cap=300): quality tied at delta R²pred=-0.0003 (off=0.9063 vs on=0.9060), passing-set diff 148 shared / 17 unique-to-OFF / 16 unique-to-ON with **arm composition difference noted** (OFF tops dominated by plain `deriv`, ON tops dominated by `snv_deriv` — both scientifically reasonable). **Wall-time, two runs:** off=15s/31s vs on=14s/14s; the rescore arm is more stable than the legacy arm because joblib JIT first-touch cache dominates the variance. So Phase 2 multi-seed rescore is **empirically tied on both quality AND wall-time** on this second dataset — the GUI label `~1.5x cost` and any "5-6x slower" framing don't survive measurement. **The 5-6x cost figure belongs to TPE multistart, not Phase 2** (`gui:11932` "~5x cost" + the 2026-05-07 -0.041 F1 classification loss = unambiguous net-negative for TPE multistart; Phase 2 is genuinely two-axis-neutral by the new rule). Recommended action: (1) leave Phase 2 plumbing as opt-in (no rip-out warranted), (2) the case for ripping out TPE multistart is unchanged and stronger by contrast.
>
> ---
>
> ## Previous session — 2026-05-08 (early) — **Both PRs merged, main is clean at `15dd011`.** PR #58 squash-merged at `46226ca` (OC hyperparameter round 2 + parser BLOCKER fix + GUI Tk worker-thread hardening + TPE multi-start top-N callback + POST-STOP prefix; multi-seed multistart default kept OFF per the 2026-05-07 postmortem revert). PR #59 squash-merged at `15dd011` (OC `decision_score_error` surfacing with GUI consumer + per-run reset; polyorder fallback fix for OC grid + derivative + validation; Model Dev `_run_refined_model_thread` Tk hardening; iPLS askyesno deadlock fix with timeout/scheduling guards). Merge gate via local diff-failure-set: PR #58 245/245, PR #59 142/142+ across all sweeps, zero failures on either branch (so zero new failures vs main is trivially proven). Cross-family + Claude-family review trail: Codex/DeepSeek/Kimi each ran multiple times across both PRs; pr-review-toolkit ran on both; every BLOCKER/MEDIUM closed or explicitly deferred with rationale. **Open follow-ups:** stale runtime label at `gui:11925` ("~1-3 min for 75 trials" — unchanged after the multi-seed default flip; one-line fix), AST-sweep test for bare-Tk-in-worker (DeepSeek/pr-review-toolkit MEDIUM, ~30 LOC `ast.parse` walk), test coverage for `_run_predictions` GUI consumer of `decision_score_error` (pr-test-analyzer CRITICAL-2; ~30 LOC with mock spy), parameterized polyorder test across deriv=0..4 (pr-test-analyzer M-2; ~15 LOC). All deferred — none affect runtime correctness today.
>
> ---
>
> ## Previous session — 2026-05-07 (evening) — **Preprocessing-discovery refactor postmortem: refactor is neutral-to-harmful at the chemometrics quality gate. GUI defaults flipped back.** New harness `tools/preprocessing_refactor_ab.py` runs the legacy-vs-refactor A/B at the user's actual filter (full leaderboard scored on external, gap filter `|R²cv − R²pred| ≤ 0.10` for "similar AND high"), set-diff at row fingerprint level. Verdict on BoneCollagen (single trajectory): **TPE multistart** classification REMOVES the user's best pick (`snv_deriv2_w15+autoscale`, F1=0.944, BAcccv=0.967, gap=0.033) from the passing set; refactor's best caps at F1=0.903 — **0.041 F1 loss**. Regression: tied within noise (R²pred 0.9680 vs 0.9687, 95% disjoint passing sets but equivalent peak). **Phase 2 multi-seed rescore in `exhaustive_search`**: regression bit-identical (115 shared, 0 unique to either arm), classification F1 tied at 0.9441. The earlier "classification benefits" finding was an artifact of measuring top-K-by-CV — a metric the user explicitly does NOT pick from because top-K are typically overfit. Defaults flipped: `ga_preprocess_phase2_rescore` GUI default `True → False`, `tpe_multistart` GUI default `True → False` (line 3242 + 3265). Plumbing kept callable so anyone explicitly opting in still gets the refactor. Postmortem memory: `feedback_preprocessing_refactor_postmortem.md` (3rd time ML-flavored search machinery shipped without chemometrics-gate validation; pattern named in `feedback_chemometrics_conventions.md` §3 but recurring). Full code rip-out (delete `phase2_adaptive_rescore` + the multistart wrapper + GUI checkboxes + result-CSV `_tpe_multistart_*` columns + tests) is a separate ~15-30 file follow-up — not done in this commit.
>
> **Previous:** 2026-05-07 — **OC hyperparameter round-2 PR open, awaiting user greenlight.** Branch `feat/oc-hyperparams-round2`, tip commit `6180749`. Exposes LOF metric (euclidean/manhattan/minkowski/cosine), LOF contamination 0.01/0.1, IF n_estimators 200/500, IF max_samples (auto/256/512), EE support_fraction (None/0.5/0.75) in Tab 4C. All 4 layers updated (curated grid, GUI vars, card UI, search.py resolver builders + collectors). 42 new tests all pass; 69 existing OC tests still pass. Three-reviewer cross-family review complete: Codex CLEAN, DeepSeek V4 Pro Max 3 MEDIUMs all by-design (curated grid expansion, IF Cartesian explosion, EE 1D→2D), Kimi BLOCKER false-positive (max_samples > n_samples is sklearn UserWarning not crash). **DO NOT MERGE** — wait for explicit user greenlight per process correction.
>
...
## [Moved block] From “Known Issues” — resolved items
...
## [Moved block] From “Known Issues” — resolved item
