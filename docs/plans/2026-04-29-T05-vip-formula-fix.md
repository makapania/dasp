...
- [ ] **Step 1.2:** Run the test against the **current (buggy)** implementation. Confirm the failures we expect.
  ...
  - `test_output_shape_and_nonnegativity` — **PASS** [SANITY] — shape and non-negativity are unaffected by the bug.
   ...
   Net (`TestVIPCanonicalFormula` only): **2 failed, 3 passed**. In `TestVIPEdgeCases`: `test_compute_vip_n_components_1` PASSES both pre-fix and post-fix (single-component PLS collapses both formulas to the same ranking); `test_compute_vip_zero_y_loadings_returns_zeros` FAILS pre-fix (the old formula uses `np.var(y) > 0` and returns nonzero VIP rather than zeros) and PASSES post-fix (the new `ssy_total <= 0` guard returns all-zeros). Pre-fix expected count across both classes: **3 failed, 4 passed**.
