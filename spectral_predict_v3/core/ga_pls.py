...
class FitnessCache:
    """
    Cache fitness evaluations to avoid redundant CV runs.
    ...
    def set(self, chromosome: np.ndarray, fitness: float):
        if len(self.cache) >= self.max_size:
            ...
            for k in keys:
                del self.cache[k]
        self.cache[chromosome.tobytes()] = fitness
...
def _fitness_function(
    ...
    try:
        if task_type == 'regression':
            ...
            # Classification: PLS-DA (PLS + LogisticRegression)
            # Simplified: use PLS to reduce dimensions, then classify
            pls = PLSRegression(n_components=actual_n_comp, scale=False)
    ...
    for gen in range(n_generations):
        ...
        if progress_callback is not None:
            ...
            progress_callback({
                ...
                'n_selected': n_sel,
                'message': f"Gen {gen + 1}/{n_generations}: fitness={best_fitness:.4f}, {n_sel} wavelengths"
            })
...
def ga_pls_selection(
    ...
    for run_idx in range(n_runs):
        ...
        best_chrom, best_fit, history = _run_single_ga(
            ...
            progress_callback if run_idx == 0 else None  # Only first run gets callbacks
        )
