"""
Interference removal transformers for spectral data (v3 standalone).

Ported from v1 - simplified for v3's numpy-first approach with sklearn compatibility.
...
class MSC(BaseEstimator, TransformerMixin):
    ...
    Examples
    --------
    >>> from spectral_predict_v3.core.interference import MSC
...
class EPO(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, X_interferents=None):
        ...
        # Validate X_interferents is provided
        ...
        # Validate interferent library has sufficient samples
        ...
        # Check for zero/low variance in interferents
        ...
        # Reduce n_components if insufficient samples
...
class GLSW(BaseEstimator, TransformerMixin):
    ...
    Examples
    --------
    >>> from spectral_predict_v3.core.interference import GLSW
    ...
    def fit(self, X, y=None):
        ...
        if self.method == 'covariance':
            ...
            # Compute variances (diagonal approximation for efficiency)
            ...
            # Compute variance of X not explained by PLS
...
class DOSC(BaseEstimator, TransformerMixin):
    ...
    Examples
    --------
    >>> from spectral_predict_v3.core.interference import DOSC
    >>> import numpy as np
    ...
    def fit(self, X, y):
        ...
        if self.n_pls_components == 'auto':
            n_pls_components = min(10, n_samples - 1, n_features)
        else:
        ...
        # Fit PLS to get Y-predictive directions
        pls = PLSRegression(n_components=n_pls_components)
        pls.fit(X_centered, y_centered)

        # Get PLS X-loadings and X-scores
        T_pls = pls.x_scores_
        P_pls = pls.x_loadings_

        # Compute residuals orthogonal to Y-predictive space
        X_reconstructed_pls = T_pls @ P_pls.T
        E_orth = X_centered - X_reconstructed_pls

        # Extract principal components of Y-orthogonal residuals
        try:
        ...
        # Take first n_components_ Y-orthogonal directions
        V = Vt.T
