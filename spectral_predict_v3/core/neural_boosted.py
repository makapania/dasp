"""Neural Boosted Regression and Classification for spectral analysis (v3).
...
class NeuralBoostedRegressor(BaseEstimator, RegressorMixin):
    ...
    learning_rate : float, default=0.1
        Shrinkage parameter (0 < v <= 1). Lower values require more estimators
...
class NeuralBoostedClassifier(BaseEstimator, ClassifierMixin):
    ...
    n_estimators_ : int or list of int
        Actual number of estimators used.
    ...
    def _fit_binary(self, X, y_binary, sample_weight=None):
        ...
        if self.early_stopping:
            ...
            min_for_stratify = int(np.ceil(1 / self.validation_fraction))
