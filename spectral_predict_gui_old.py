...
class SpectralPredictApp:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("1200x900")  # Larger window for better fit

        # Configure modern art gallery-style theme
        ...
        # Interactive tab variables
        self.interactive_tab_created = False
    ...
    def _configure_style(self):
        ...
        # Art gallery color palette - sophisticated and minimal
        self.colors = {
            'bg': '#F5F5F5',          # Soft white background
            'panel': '#FFFFFF',        # Pure white for panels
            'text': '#2C3E50',         # Deep blue-gray for text
            'text_light': '#7F8C8D',   # Lighter gray for secondary text
    ...
    def _create_ui(self):
        ...
        # Main container with padding - using canvas for scrolling if needed
        canvas = tk.Canvas(self.config_tab, bg=self.colors['bg'], highlightthickness=0)
        ...
        title = ttk.Label(main_frame, text="Spectral Predict",
                         style='Title.TLabel')
        title.grid(row=0, column=0, columnspan=3, pady=(0, 5), sticky=tk.W)

        subtitle = ttk.Label(main_frame,
                            text="Automated spectral analysis with preprocessing and model ranking",
                            style='Caption.TLabel')
        subtitle.grid(row=1, column=0, columnspan=3, pady=(0, 3), sticky=tk.W)

        models_info = ttk.Label(main_frame,
                               text="Models: PLS • Random Forest • MLP • Neural Boosted",
                               style='Caption.TLabel')
        models_info.grid(row=2, column=0, columnspan=3, pady=(0, 20), sticky=tk.W)

        # Add subtle separator
        separator = ttk.Separator(main_frame, orient='horizontal')
        separator.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 20))
        ...
        section_label.grid(row=current_row, column=0, columnspan=3, sticky=tk.W, pady=(0, 15))
        current_row += 1
        ...
        section_label = ttk.Label(main_frame, text="2. Column Names (from Reference CSV)",
                                 style='Heading.TLabel')
        section_label.grid(row=current_row, column=0, columnspan=3, sticky=tk.W, pady=(25, 15))
        current_row += 1
